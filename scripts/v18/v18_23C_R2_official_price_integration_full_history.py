...
STATUS_OK = "OK_V18_23C_R2_OFFICIAL_FULL_HISTORY_INTEGRATION_READY"
STATUS_WARN = "WARN_V18_23C_R2_OFFICIAL_INTEGRATION_PARTIAL"
STATUS_FAIL = "FAIL_V18_23C_R2_OFFICIAL_INTEGRATION"
MODE = "OFFICIAL_PRICE_CACHE_INTEGRATION_FULL_HISTORY_ONLY_WITH_BACKUP"
BATCH_ID = "V18_23C_BATCH1"
STAGED_DIR_REL = "data/v18/staged_backfill/V18_23C_BATCH1"
DEST_DIR_REL = "state/v18/price_cache"

OUTPUTS = {
    "md": "outputs/v18/staged_backfill/V18_23C_R2_CURRENT_OFFICIAL_INTEGRATION.md",
    "candidates": "outputs/v18/staged_backfill/V18_23C_R2_CURRENT_FULL_HISTORY_INTEGRATION_CANDIDATES.csv",
    "destination": "outputs/v18/staged_backfill/V18_23C_R2_CURRENT_OFFICIAL_DESTINATION_AUDIT.csv",
    "backup": "outputs/v18/staged_backfill/V18_23C_R2_CURRENT_BACKUP_MANIFEST.csv",
    "merge": "outputs/v18/staged_backfill/V18_23C_R2_CURRENT_MERGE_RESULT.csv",
    "held": "outputs/v18/staged_backfill/V18_23C_R2_CURRENT_HELD_OUT_TICKERS.csv",
    "retest": "outputs/v18/rolling_coverage/V18_23C_R2_CURRENT_POST_INTEGRATION_RETEST.csv",
    "retest_summary": "outputs/v18/rolling_coverage/V18_23C_R2_CURRENT_POST_INTEGRATION_RETEST_SUMMARY.csv",
    "validation": "outputs/v18/staged_backfill/V18_23C_R2_CURRENT_VALIDATION.csv",
    "read_first": "outputs/v18/ops/V18_23C_R2_READ_FIRST.txt",
    "report": "outputs/v18/ops/V18_23C_R2_CURRENT_OFFICIAL_INTEGRATION_REPORT.md",
}

READ_FIRST_FIELDS = [
    "STATUS", "MODE", "OFFICIAL_INTEGRATION_READY", "BATCH_ID", "INTEGRATION_SCOPE",
...
def normalize_price_row(row: Dict[str, str], source_file: str, updated_at: str) -> Dict[str, str]:
    ...
    return {
        ...
        "source": "V18_23C_R2_STAGED_FULL_HISTORY_INTEGRATION",
...
def render_restore_script(backup_rows: Sequence[Dict[str, object]]) -> str:
    lines = [
        ...
        'Write-Host "=== RESTORE V18.23C-R2 PRICE CACHE START ==="',
    ]
    ...
    lines += ['Write-Host "=== RESTORE V18.23C-R2 PRICE CACHE END ==="', 'exit 0']
    return "\n".join(lines) + "\n"
...
def render_md(values: Dict[str, str]) -> str:
    return f"""# V18.23C-R2 Official Price Cache Integration
...
def main() -> int:
    ...
    backup_dir = root / "archive/v18/price_cache_backups" / f"V18_23C_R2_{timestamp}"
    restore_path = backup_dir / "RESTORE_V18_23C_R2_PRICE_CACHE.ps1"
    backup_manifest_path = backup_dir / "BACKUP_MANIFEST.csv"

    merge_input, _ = read_csv(root / "outputs/v18/staged_backfill/V18_23C_R1_CURRENT_MERGE_CANDIDATES.csv")
    quality_rows, _ = read_csv(root / "outputs/v18/staged_backfill/V18_23C_R1_CURRENT_TICKER_QUALITY_AUDIT.csv")
    full_candidates = [row for row in merge_input if row.get("classification") == "MERGE_CANDIDATE_FULL_HISTORY"]
    ...
    values: Dict[str, str] = {
        "STATUS": STATUS_OK,
        "MODE": MODE,
        "OFFICIAL_INTEGRATION_READY": "FALSE",
        "BATCH_ID": BATCH_ID,
    ...
    validations = [
        validation_row("python_compile_check", py_compile(root / "scripts/v18/v18_23C_R2_official_price_integration_full_history.py"), 1, "Python compile."),
        validation_row("powershell_parse_check", ps_parse(root / "scripts/v18/run_v18_23C_R2_official_price_integration_full_history.ps1"), 1, "PowerShell parse."),
        validation_row("required_outputs_exist_non_empty", all(non_empty(path) for path in required_outputs), 1, "All R2 outputs must exist and be non-empty."),
        validation_row("official_destination_found", destination_found, 1, "Destination must be safely identified."),
        validation_row("candidate_count_matches_r1", len(full_tickers) == 51, 1, f"Expected 51 from R1; actual {len(full_tickers)}."),
        validation_row("held_out_contains_exclusions", bool(held_rows) and int(values["PRICE_ONLY_PARTIAL_EXCLUDED_COUNT"]) == 5 and int(values["EMPTY_FETCH_EXCLUDED_COUNT"]) == 2, 1, "Held out must include partial and empty fetch tickers."),
    ...
    values["OFFICIAL_INTEGRATION_READY"] = "TRUE" if values["STATUS"] != STATUS_FAIL else "FALSE"
