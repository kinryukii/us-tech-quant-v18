...
MODE = "SNAPSHOT_ONLY"
STATUS_OK = "OK_V18_12F_R2_STABLE_SNAPSHOT_READY"
STATUS_WARN = "WARN_V18_12F_R2_STABLE_SNAPSHOT_WITH_FAILURES"
SHADOW_ONLY = "TRUE"
...
Write-Host ""
Write-Host "=== V18.12F-R2 STABLE SNAPSHOT RESTORE ==="
...
def build_readme(snapshot: Path, manifest_path: Path, validation_path: Path, restore_path: Path) -> str:
    return f"""V18.12F-R2 STABLE SNAPSHOT README
...
Purpose:
Stable restore point for V18.12F Shadow Research Daily With Sell Timing Integration after the V18.12E hard-fail guard was added.
...
MISSING_DEPENDENCY_FAILURE_STATUS=FAIL_V18_12F_SELL_TIMING_DEPENDENCY_MISSING
...
def main() -> int:
    ...
    stamp = now_stamp()
    snapshot = root / "archive/stable" / f"V18_12F_R2_stable_shadow_research_daily_with_sell_timing_hard_fail_guard_{stamp}"
    ...
    manifest_path = snapshot / "V18_12F_R2_STABLE_MANIFEST.csv"
    validation_path = snapshot / "V18_12F_R2_STABLE_VALIDATION_CHECKS.csv"
    readme_path = snapshot / "V18_12F_R2_STABLE_SNAPSHOT_README.txt"
    restore_path = snapshot / "restore_v18_12F_R2_stable_snapshot.ps1"
    report_path = out_dir / "V18_12F_R2_CURRENT_STABLE_SNAPSHOT_REPORT.md"
    read_first_path = out_dir / "V18_12F_R2_READ_FIRST.txt"
    ...
    wrapper_text = read_text(root / "scripts/v18/run_v18_12F_shadow_research_daily_with_sell_timing.ps1")
    read_center_text = read_text(root / "scripts/v18/v18_12F_shadow_research_sell_timing_read_center.py")
    hard_fail_wrapper_ok = "MISSING_CRITICAL_V18_12E_SELL_TIMING_DAILY_WRAPPER" in wrapper_text and "$SellTimingExitCode = 12" in wrapper_text
    hard_fail_read_center_ok = "FAIL_V18_12F_SELL_TIMING_DEPENDENCY_MISSING" in read_center_text
    validations.append(
        validation_row(
            "V18_12F_HARD_FAIL_WRAPPER_GUARD_PRESENT",
            "PASS" if hard_fail_wrapper_ok else "FAIL",
            root / "scripts/v18/run_v18_12F_shadow_research_daily_with_sell_timing.ps1",
            "missing_v18_12e_sets_nonzero_exit",
            "present" if hard_fail_wrapper_ok else "missing",
        )
    )
    validations.append(
        validation_row(
            "V18_12F_HARD_FAIL_READ_CENTER_STATUS_PRESENT",
            "PASS" if hard_fail_read_center_ok else "FAIL",
            root / "scripts/v18/v18_12F_shadow_research_sell_timing_read_center.py",
            "failure_status_present",
            "present" if hard_fail_read_center_ok else "missing",
        )
    )
    ...
    report = f"""# V18.12F-R2 Stable Snapshot
...
The V18.12F wrapper was run in fast safe mode before copying. PowerShell parse checks, Python compile checks, critical file copy checks, protected-file hash checks, V18.12A/B/C/D/E/F-R1 stable snapshot manifest hash checks, and generated output token checks were recorded.
"""
    report_path.write_text(report, encoding="utf-8")

    read_first_out = f"""V18.12F-R2 STABLE SNAPSHOT READ FIRST
