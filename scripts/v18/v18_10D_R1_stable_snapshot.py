...
def build_restore_script(snapshot: Path, restore_path: Path) -> None:
    ...
    lines.append('Write-Host "=== V18.10D-R1 STABLE SNAPSHOT RESTORE ==="')
...
def main() -> int:
    ...
    stamp = now_stamp()
    snapshot_name = f"V18_10D_R1_stable_official_daily_with_factor_weight_research_{stamp}"
    ...
    manifest_path = snapshot / "V18_10D_R1_STABLE_MANIFEST.csv"
    readme_path = snapshot / "V18_10D_R1_STABLE_SNAPSHOT_README.txt"
    restore_path = snapshot / "restore_v18_10D_R1_stable_snapshot.ps1"
    check_path = snapshot / "V18_10D_R1_STABLE_VALIDATION_CHECKS.csv"
    combined_rf_path = snapshot / "V18_10D_R1_COMBINED_READ_FIRST_CAPTURE.txt"

    report_path = out_dir / "V18_10D_R1_CURRENT_STABLE_SNAPSHOT_REPORT.md"
    read_first_path = out_dir / "V18_10D_R1_READ_FIRST.txt"
    ...
    critical_py = [
        ...
        "scripts/v18/v18_10D_R1_stable_snapshot.py",
    ]
    ...
    readme_lines = []
    readme_lines.append("V18.10D-R1 STABLE SNAPSHOT README")
    readme_lines.append("")
    ...
    readme_lines.append("Stable restore point after V18.10D official daily plus shadow factor/weight research wrapper passed light test.")
    readme_lines.append("")
    ...
    report_lines = []
    report_lines.append("# V18.10D-R1 Stable Snapshot")
    report_lines.append("")
    ...
    report_lines.append("- MODE: `STABLE_RESTORE_POINT_FOR_OFFICIAL_DAILY_WITH_SHADOW_FACTOR_WEIGHT_RESEARCH`")
    ...
    read_first = f"""V18.10D-R1 STABLE SNAPSHOT READ FIRST

STATUS:
{status}

MODE:
STABLE_RESTORE_POINT_FOR_OFFICIAL_DAILY_WITH_SHADOW_FACTOR_WEIGHT_RESEARCH
...
NEXT_STEP:
If STATUS is OK_STABLE_SNAPSHOT_READY, keep V18.10D as a shadow research extension.
Do not point run_v18_current_official_daily.ps1 to V18.10D unless separately approved.
...
"""
    ...
    print("")
    print("=== V18.10D-R1 STABLE SNAPSHOT READY ===")
    print(f"STATUS: {status}")
    print("MODE: STABLE_RESTORE_POINT_FOR_OFFICIAL_DAILY_WITH_SHADOW_FACTOR_WEIGHT_RESEARCH")
