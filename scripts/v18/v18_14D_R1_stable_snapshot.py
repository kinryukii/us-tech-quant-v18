...
STATUS_OK = "OK_V18_14D_R1_STABLE_SNAPSHOT_READY"
STATUS_WARN = "WARN_V18_14D_R1_STABLE_SNAPSHOT_WITH_FAILURES"
STATUS_FAIL = "FAIL_V18_14D_R1_STABLE_SNAPSHOT"

EXPECTED_CURRENT_STATUS = "OK_V18_14D_RANKED_CANDIDATE_FORWARD_PRICE_FILLER_READY"
EXPECTED_TRACKER_ROWS = "20"
EXPECTED_PRICE_SOURCE_STATUS = "LOCAL_PRICE_AVAILABLE"
EXPECTED_PENDING_FORWARD_ROWS = "20"
EXPECTED_PENDING_SIGNAL_PRICE_ROWS = "0"
...
SNAPSHOT_PREFIX = "V18_14D_R1_stable_ranked_candidate_forward_price_filler"

SNAPSHOT_FILES = [
    ("script", "scripts/v18/run_v18_14D_ranked_candidate_forward_price_filler.ps1", True),
    ("script", "scripts/v18/v18_14D_ranked_candidate_forward_price_filler.py", True),
    ("script", "scripts/v18/run_v18_14D_R1_stable_snapshot.ps1", True),
    ("script", "scripts/v18/v18_14D_R1_stable_snapshot.py", True),
    ("state", "state/v18/candidate_forward_tracker/V18_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER.csv", True),
    ("v18_14d_output", "outputs/v18/candidates/V18_14D_CURRENT_RANKED_CANDIDATE_FORWARD_PRICE_FILLER.csv", True),
    ("v18_14d_output", "outputs/v18/candidates/V18_14D_CURRENT_RANKED_CANDIDATE_FORWARD_PRICE_FILLER_AUDIT.csv", True),
    ("v18_14d_output", "outputs/v18/ops/V18_14D_READ_FIRST.txt", True),
    ("v18_14d_output", "outputs/v18/ops/V18_14D_CURRENT_RANKED_CANDIDATE_FORWARD_PRICE_FILLER_REPORT.md", True),
    ("v18_14d_output", "outputs/v18/ops/V18_14D_CURRENT_RANKED_CANDIDATE_FORWARD_PRICE_FILLER_SUMMARY.csv", True),
    ("v18_14d_output", "outputs/v18/ops/V18_14D_CURRENT_RANKED_CANDIDATE_FORWARD_PRICE_FILLER_INPUT_AUDIT.csv", True),
    ...
    ("upstream", "outputs/v18/ops/V18_14C_R1_READ_FIRST.txt", False),
    ("upstream", "outputs/v18/candidates/V18_14C_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER.csv", False),
    ("upstream", "outputs/v18/candidates/V18_CURRENT_RANKED_CANDIDATES.csv", False),
    ("upstream", "outputs/v18/ops/V18_14B_R1_READ_FIRST.txt", False),
]

PS_PARSE_FILES = [
    "scripts/v18/run_v18_14D_ranked_candidate_forward_price_filler.ps1",
    "scripts/v18/run_v18_14D_R1_stable_snapshot.ps1",
]

PY_COMPILE_FILES = [
    "scripts/v18/v18_14D_ranked_candidate_forward_price_filler.py",
    "scripts/v18/v18_14D_R1_stable_snapshot.py",
]
...
def build_restore_script(snapshot: Path) -> str:
    restore_files = [
        ...
        if category in {"script", "state", "v18_14d_output"} and required
    ]
...
Write-Host "=== V18.14D-R1 STABLE SNAPSHOT RESTORE ==="
...
if (-not $Apply) {{
    Write-Host "DRYRUN_ONLY. Re-run with -Apply to restore V18.14D-R1 price filler files."
}}
else {{
...
def build(root: Path) -> Tuple[Dict[str, str], int]:
    ...
    readme_path = snapshot / "README_V18_14D_R1_STABLE_SNAPSHOT.md"
    restore_path = snapshot / "RESTORE_V18_14D_R1.ps1"
    report_path = ops_dir / "V18_14D_R1_CURRENT_STABLE_SNAPSHOT_REPORT.md"
    read_first_path = ops_dir / "V18_14D_R1_READ_FIRST.txt"

    current_read_first = root / "outputs/v18/ops/V18_14D_READ_FIRST.txt"
    ...
    current_values = {
        ...
        "PARTIAL_FORWARD_FILLED_ROWS": first_value(current_read_first, "PARTIAL_FORWARD_FILLED_ROWS"),
        "PENDING_FORWARD_ROWS": first_value(current_read_first, "PENDING_FORWARD_ROWS"),
        "PENDING_SIGNAL_PRICE_ROWS": first_value(current_read_first, "PENDING_SIGNAL_PRICE_ROWS"),
        "PRICE_SOURCE_COUNT": first_value(current_read_first, "PRICE_SOURCE_COUNT"),
    ...
    expected = {
        ...
        "PENDING_SIGNAL_PRICE_ROWS": EXPECTED_PENDING_SIGNAL_PRICE_ROWS,
    ...
    for key, expected_value in expected.items():
        ...
        validations.append(validation_row(f"CURRENT_V18_14D_{key}", "PASS" if actual == expected_value else "FAIL", current_read_first, expected_value, actual))
    validations.append(validation_row("TRACKER_DEDUP_SIGNAL_DATE_TICKER_RANK_SOURCE", "PASS" if duplicate_count == 0 else "FAIL", tracker_path, "0", str(duplicate_count)))
    ...
    readme = f"""# V18.14D-R1 Stable Snapshot

Created: {now_text()}

Snapshot: {snapshot}

Purpose: Stable restore point for the V18.14D ranked candidate forward price filler.

Scope: Snapshot only. This does not run V18.14D, does not modify V18.13D, V18.14A/B/C, official daily, or existing stable snapshots.
...
PARTIAL_FORWARD_FILLED_ROWS: {current_values['PARTIAL_FORWARD_FILLED_ROWS']}
PENDING_FORWARD_ROWS: {current_values['PENDING_FORWARD_ROWS']}
PENDING_SIGNAL_PRICE_ROWS: {current_values['PENDING_SIGNAL_PRICE_ROWS']}
PRICE_SOURCE_COUNT: {current_values['PRICE_SOURCE_COUNT']}
...
"""
    ...
    scan_paths = [
        root / "state/v18/candidate_forward_tracker/V18_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER.csv",
        root / "outputs/v18/candidates/V18_14D_CURRENT_RANKED_CANDIDATE_FORWARD_PRICE_FILLER.csv",
        root / "outputs/v18/candidates/V18_14D_CURRENT_RANKED_CANDIDATE_FORWARD_PRICE_FILLER_AUDIT.csv",
        root / "outputs/v18/ops/V18_14D_READ_FIRST.txt",
        root / "outputs/v18/ops/V18_14D_CURRENT_RANKED_CANDIDATE_FORWARD_PRICE_FILLER_REPORT.md",
        root / "outputs/v18/ops/V18_14D_CURRENT_RANKED_CANDIDATE_FORWARD_PRICE_FILLER_SUMMARY.csv",
        root / "outputs/v18/ops/V18_14D_CURRENT_RANKED_CANDIDATE_FORWARD_PRICE_FILLER_INPUT_AUDIT.csv",
        snapshot / "state/v18/candidate_forward_tracker/V18_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER.csv",
        snapshot / "outputs/v18/candidates/V18_14D_CURRENT_RANKED_CANDIDATE_FORWARD_PRICE_FILLER.csv",
        snapshot / "outputs/v18/candidates/V18_14D_CURRENT_RANKED_CANDIDATE_FORWARD_PRICE_FILLER_AUDIT.csv",
        snapshot / "outputs/v18/ops/V18_14D_READ_FIRST.txt",
        snapshot / "outputs/v18/ops/V18_14D_CURRENT_RANKED_CANDIDATE_FORWARD_PRICE_FILLER_REPORT.md",
        snapshot / "outputs/v18/ops/V18_14D_CURRENT_RANKED_CANDIDATE_FORWARD_PRICE_FILLER_SUMMARY.csv",
        snapshot / "outputs/v18/ops/V18_14D_CURRENT_RANKED_CANDIDATE_FORWARD_PRICE_FILLER_INPUT_AUDIT.csv",
        snapshot / "outputs/v18/ops/V18_14C_READ_FIRST.txt",
        snapshot / "outputs/v18/ops/V18_14C_R1_READ_FIRST.txt",
        snapshot / "outputs/v18/candidates/V18_14C_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER.csv",
        snapshot / "outputs/v18/candidates/V18_CURRENT_RANKED_CANDIDATES.csv",
        snapshot / "outputs/v18/ops/V18_14B_R1_READ_FIRST.txt",
        readme_path,
        restore_path,
    ]
    hits = dangerous_hits(root, scan_paths)
    validations.append(validation_row("NO_DANGEROUS_TOKEN_IN_V18_14D_OUTPUTS_AND_SNAPSHOT", "PASS" if not hits else "FAIL", "V18.14D outputs and snapshot artifacts", "clean", "clean" if not hits else "found", "NONE" if not hits else ";".join(hits[:10])))
    ...
    read_first_keys = [
        ...
        "VALIDATION_FAIL_COUNT", "TRACKER_ROWS", "UPDATED_FORWARD_ROWS", "FORWARD_COMPLETE_ROWS",
        "PARTIAL_FORWARD_FILLED_ROWS", "PENDING_FORWARD_ROWS", "PENDING_SIGNAL_PRICE_ROWS",
        "PRICE_SOURCE_COUNT", "PRICE_SOURCE_STATUS", "TOP_5_TICKERS", "OFFICIAL_DECISION_IMPACT",
        "AUTO_TRADE", "AUTO_SELL", "SNAPSHOT_ONLY", "DANGEROUS_TOKEN_DETECTED",
        "DUPLICATE_PREVENTION_VALIDATED", "PROTECTED_STABLE_SNAPSHOTS_UNCHANGED",
    ]
    ...
    for key in [
        ...
        "PARTIAL_FORWARD_FILLED_ROWS", "PENDING_FORWARD_ROWS", "PENDING_SIGNAL_PRICE_ROWS",
        "PRICE_SOURCE_COUNT", "PRICE_SOURCE_STATUS", "TOP_5_TICKERS", "OFFICIAL_DECISION_IMPACT",
        "AUTO_TRADE", "AUTO_SELL", "SNAPSHOT_ONLY",
    ]:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create V18.14D-R1 ranked candidate forward price filler stable snapshot.")
