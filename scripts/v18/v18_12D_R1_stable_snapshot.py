...
MODE = "SNAPSHOT_ONLY"
STATUS_OK = "OK_V18_12D_R1_STABLE_SNAPSHOT_READY"
OFFICIAL_DECISION_IMPACT = "NONE"
AUTO_SELL = "DISABLED"
AUTO_TRADE = "DISABLED"
OFFICIAL_DAILY_MODIFIED = "False"
V18_12A_R1_SNAPSHOT_MODIFIED = "False"
V18_12B_R1_SNAPSHOT_MODIFIED = "False"
V18_12C_R1_SNAPSHOT_MODIFIED = "False"
SHADOW_ONLY = "True"

SNAPSHOT_FILES = [
    ("script", "scripts/v18/v18_12D_exit_signal_forward_validation.py"),
    ("script", "scripts/v18/run_v18_12D_exit_signal_forward_validation.ps1"),
    ("output", "outputs/v18/sell_timing/V18_12D_READ_FIRST.txt"),
    ("output", "outputs/v18/sell_timing/V18_12D_CURRENT_EXIT_SIGNAL_FORWARD_VALIDATION_REPORT.md"),
    ("output", "outputs/v18/sell_timing/V18_12D_CURRENT_EXIT_SIGNAL_FORWARD_VALIDATION.csv"),
    ("output", "outputs/v18/sell_timing/V18_12D_CURRENT_EXIT_SIGNAL_FORWARD_VALIDATION_INPUT_AUDIT.csv"),
    ("state", "state/v18/sell_timing/V18_CURRENT_EXIT_SIGNAL_FORWARD_TRACKER.csv"),
    ("output", "outputs/v18/sell_timing/V18_CURRENT_EXIT_SIGNAL_FORWARD_TRACKER.csv"),
]
...
PY_COMPILE_FILES = [
    "scripts/v18/v18_12D_exit_signal_forward_validation.py",
    "scripts/v18/v18_12D_R1_stable_snapshot.py",
]

V18_12D_OUTPUTS = [
    "outputs/v18/sell_timing/V18_12D_READ_FIRST.txt",
    "outputs/v18/sell_timing/V18_12D_CURRENT_EXIT_SIGNAL_FORWARD_VALIDATION_REPORT.md",
    "outputs/v18/sell_timing/V18_12D_CURRENT_EXIT_SIGNAL_FORWARD_VALIDATION.csv",
    "outputs/v18/sell_timing/V18_CURRENT_EXIT_SIGNAL_FORWARD_TRACKER.csv",
    "state/v18/sell_timing/V18_CURRENT_EXIT_SIGNAL_FORWARD_TRACKER.csv",
]
...
def run_v18_12d_wrapper(root: Path) -> Tuple[bool, str]:
    wrapper = root / "scripts/v18/run_v18_12D_exit_signal_forward_validation.ps1"
    try:
        proc = subprocess.run(
...
def stable_snapshot_manifest_hashes(snapshot_root: Path, manifest_name: str) -> Tuple[bool, str, int]:
    ...
    for row in rows:
        if row.get("status") != "COPIED":
            continue
        if row.get("relative_snapshot_path") == manifest_name:
            continue
...
def all_snapshots_unchanged(root: Path, pattern: str, manifest_name: str) -> Tuple[bool, str, int]:
    checked = 0
    problems: List[str] = []
    for snapshot_root in (root / "archive/stable").glob(pattern):
...
def no_order_tokens_in_outputs(root: Path) -> Tuple[bool, str]:
    hits: List[str] = []
    for rel_path in V18_12D_OUTPUTS:
        path = root / rel_path
...
Write-Host ""
Write-Host "=== V18.12D-R1 STABLE SNAPSHOT RESTORE ==="
Write-Host "SNAPSHOT: $Snapshot"
Write-Host "ROOT: $Root"
Write-Host "APPLY: $Apply"
Write-Host "NOTE: Restore covers only V18.12D exit signal forward validation files."
...
def build_readme(snapshot: Path, manifest_path: Path, validation_path: Path, restore_path: Path) -> str:
    return f"""V18.12D-R1 STABLE SNAPSHOT README
...
Purpose:
Stable restore point for the additive V18.12D Exit Signal Forward Validation layer.

Scope:
Copies the V18.12D validation engine, wrapper, READ_FIRST, report, validation CSV, input audit CSV, and persistent/output tracker files.
...
V18_12B_R1_SNAPSHOT_MODIFIED={V18_12B_R1_SNAPSHOT_MODIFIED}
V18_12C_R1_SNAPSHOT_MODIFIED={V18_12C_R1_SNAPSHOT_MODIFIED}

Restore:
The restore script is generated but not executed by snapshot creation. It restores only V18.12D exit signal forward validation files.
...
def main() -> int:
    ...
    stamp = now_stamp()
    snapshot = root / "archive/stable" / f"V18_12D_R1_stable_exit_signal_forward_validation_{stamp}"
    ...
    manifest_path = snapshot / "V18_12D_R1_STABLE_MANIFEST.csv"
    validation_path = snapshot / "V18_12D_R1_STABLE_VALIDATION_CHECKS.csv"
    readme_path = snapshot / "V18_12D_R1_STABLE_SNAPSHOT_README.txt"
    restore_path = snapshot / "restore_v18_12D_R1_stable_snapshot.ps1"
    report_path = out_dir / "V18_12D_R1_CURRENT_STABLE_SNAPSHOT_REPORT.md"
    read_first_path = out_dir / "V18_12D_R1_READ_FIRST.txt"

    validations: List[Dict[str, str]] = []
    wrapper_ok, wrapper_output = run_v18_12d_wrapper(root)
    validations.append(validation_row("RUN_V18_12D_WRAPPER", "PASS" if wrapper_ok else "FAIL", root / "scripts/v18/run_v18_12D_exit_signal_forward_validation.ps1", "exit_0", "exit_0" if wrapper_ok else "nonzero", wrapper_output))
    ...
    read_first = root / "outputs/v18/sell_timing/V18_12D_READ_FIRST.txt"
    for key, expected in [
        ...
        validations.append(validation_row(f"V18_12D_{key}", "PASS" if actual == expected else "FAIL", read_first, expected, actual))

    no_token_ok, token_note = no_order_tokens_in_outputs(root)
    validations.append(validation_row("NO_ORDER_TOKEN_IN_V18_12D_OUTPUTS", "PASS" if no_token_ok else "FAIL", "outputs/state V18.12D tracker files", "no order tokens", "clean" if no_token_ok else "found", token_note))
    ...
    a_ok, a_note, a_checked = all_snapshots_unchanged(root, "V18_12A_R1_stable*", "V18_12A_R1_STABLE_MANIFEST.csv")
    validations.append(validation_row("V18_12A_R1_STABLE_SNAPSHOT_UNCHANGED", "PASS" if a_ok else "FAIL", root / "archive/stable", "manifest_hashes_match", f"checked={a_checked}" if a_ok else "mismatch", a_note))

    b_ok, b_note, b_checked = all_snapshots_unchanged(root, "V18_12B_R1_stable*", "V18_12B_R1_STABLE_MANIFEST.csv")
    validations.append(validation_row("V18_12B_R1_STABLE_SNAPSHOT_UNCHANGED", "PASS" if b_ok else "FAIL", root / "archive/stable", "manifest_hashes_match", f"checked={b_checked}" if b_ok else "mismatch", b_note))

    c_ok, c_note, c_checked = all_snapshots_unchanged(root, "V18_12C_R1_stable*", "V18_12C_R1_STABLE_MANIFEST.csv")
    validations.append(validation_row("V18_12C_R1_STABLE_SNAPSHOT_UNCHANGED", "PASS" if c_ok else "FAIL", root / "archive/stable", "manifest_hashes_match", f"checked={c_checked}" if c_ok else "mismatch", c_note))
    ...
    status = STATUS_OK if fail_count == 0 else "WARN_V18_12D_R1_STABLE_SNAPSHOT_WITH_FAILURES"

    report = f"""# V18.12D-R1 Stable Snapshot
...
The snapshot contains only V18.12D exit signal forward validation files, generated V18.12D outputs, persistent/output trackers, README, manifest, validation checks, and restore tooling.
...
The V18.12D wrapper was run before copying. PowerShell parse checks, Python compile checks, critical file copy checks, official daily hash check, V18.12A/B/C stable snapshot manifest hash checks, and generated output order-token checks were recorded.
"""
    report_path.write_text(report, encoding="utf-8")

    read_first_out = f"""V18.12D-R1 STABLE SNAPSHOT READ FIRST
...
V18_12B_R1_SNAPSHOT_MODIFIED:
{V18_12B_R1_SNAPSHOT_MODIFIED}

V18_12C_R1_SNAPSHOT_MODIFIED:
{V18_12C_R1_SNAPSHOT_MODIFIED}
...
"""
    ...
    print(f"V18_12B_R1_SNAPSHOT_MODIFIED: {V18_12B_R1_SNAPSHOT_MODIFIED}")
    print(f"V18_12C_R1_SNAPSHOT_MODIFIED: {V18_12C_R1_SNAPSHOT_MODIFIED}")
