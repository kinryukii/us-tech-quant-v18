...
MODE = "SNAPSHOT_ONLY"
STATUS_OK = "OK_V18_13A_R1_STABLE_SNAPSHOT_READY"
STATUS_WARN = "WARN_V18_13A_R1_STABLE_SNAPSHOT_WITH_FAILURES"
READ_LINK_ONLY = "TRUE"
OFFICIAL_DECISION_IMPACT = "NONE"
AUTO_SELL = "DISABLED"
AUTO_TRADE = "DISABLED"
EXPECTED_UNIFIED_STATUS = "OK_LINKED_CURRENT_READ_FILES"
EXPECTED_13A_STATUS = "OK_V18_13A_UNIFIED_DAILY_READ_CENTER_READY"

SNAPSHOT_FILES = [
    ("script", "scripts/v18/run_v18_13A_unified_daily_read_center_link.ps1"),
    ("script", "scripts/v18/v18_13A_unified_daily_read_center_link.py"),
    ("output", "outputs/v18/read_center/V18_13A_READ_FIRST.txt"),
    ("output", "outputs/v18/read_center/V18_13A_CURRENT_UNIFIED_DAILY_READ_CENTER.md"),
    ("output", "outputs/v18/read_center/V18_13A_CURRENT_UNIFIED_DAILY_SUMMARY.csv"),
    ("output", "outputs/v18/read_center/V18_13A_CURRENT_UNIFIED_DAILY_INPUT_AUDIT.csv"),
]

PS_PARSE_FILES = [
    "scripts/v18/run_v18_13A_unified_daily_read_center_link.ps1",
    "scripts/v18/run_v18_13A_R1_stable_snapshot.ps1",
]

PY_COMPILE_FILES = [
    "scripts/v18/v18_13A_unified_daily_read_center_link.py",
    "scripts/v18/v18_13A_R1_stable_snapshot.py",
]
...
V18_13A_OUTPUTS = [
    "outputs/v18/read_center/V18_13A_READ_FIRST.txt",
    "outputs/v18/read_center/V18_13A_CURRENT_UNIFIED_DAILY_READ_CENTER.md",
    "outputs/v18/read_center/V18_13A_CURRENT_UNIFIED_DAILY_SUMMARY.csv",
    "outputs/v18/read_center/V18_13A_CURRENT_UNIFIED_DAILY_INPUT_AUDIT.csv",
]
...
def run_v18_13a_link(root: Path) -> Tuple[bool, str]:
    wrapper = root / "scripts/v18/run_v18_13A_unified_daily_read_center_link.ps1"
    try:
        proc = subprocess.run(
            [
                "powershell",
...
def no_order_tokens_in_outputs(root: Path) -> Tuple[bool, str]:
    hits: List[str] = []
    for rel_path in V18_13A_OUTPUTS:
        path = root / rel_path
...
Write-Host ""
Write-Host "=== V18.13A-R1 STABLE SNAPSHOT RESTORE ==="
Write-Host "SNAPSHOT: $Snapshot"
Write-Host "ROOT: $Root"
Write-Host "APPLY: $Apply"
Write-Host "NOTE: Restore covers only V18.13A unified daily read center link files and outputs."
...
def build_readme(snapshot: Path, manifest_path: Path, validation_path: Path, restore_path: Path) -> str:
    return f"""V18.13A-R1 STABLE SNAPSHOT README
...
Purpose:
Stable restore point for V18.13A Unified Daily Read Center Link after linking official daily, shadow research, sell timing, cleanup audit, and stable baseline statuses.

Scope:
Copies the V18.13A wrapper, V18.13A read-link script, READ_FIRST, unified read center Markdown, summary CSV, and input audit CSV.

Safety:
MODE={MODE}
READ_LINK_ONLY={READ_LINK_ONLY}
OFFICIAL_DECISION_IMPACT={OFFICIAL_DECISION_IMPACT}
AUTO_SELL={AUTO_SELL}
AUTO_TRADE={AUTO_TRADE}
UNIFIED_READ_CENTER_STATUS={EXPECTED_UNIFIED_STATUS}

Restore:
The restore script is generated but not executed by snapshot creation. It restores only V18.13A files and generated V18.13A outputs.
...
def main() -> int:
    ...
    stamp = now_stamp()
    snapshot = root / "archive/stable" / f"V18_13A_R1_stable_unified_daily_read_center_link_{stamp}"
    ...
    manifest_path = snapshot / "V18_13A_R1_STABLE_MANIFEST.csv"
    validation_path = snapshot / "V18_13A_R1_STABLE_VALIDATION_CHECKS.csv"
    readme_path = snapshot / "V18_13A_R1_STABLE_SNAPSHOT_README.txt"
    restore_path = snapshot / "restore_v18_13A_R1_stable_snapshot.ps1"
    report_path = out_dir / "V18_13A_R1_CURRENT_STABLE_SNAPSHOT_REPORT.md"
    read_first_path = out_dir / "V18_13A_R1_READ_FIRST.txt"
    read_first_13a = root / "outputs/v18/read_center/V18_13A_READ_FIRST.txt"

    validations: List[Dict[str, str]] = []
    wrapper_ok, wrapper_output = run_v18_13a_link(root)
    validations.append(validation_row("RUN_V18_13A_UNIFIED_DAILY_READ_CENTER_LINK", "PASS" if wrapper_ok else "FAIL", root / "scripts/v18/run_v18_13A_unified_daily_read_center_link.ps1", "exit_0", "exit_0" if wrapper_ok else "nonzero", wrapper_output))
    ...
    values_13a: Dict[str, str] = {}
    for key, expected in read_first_expectations:
        actual = read_first_value(read_first_13a, key)
        values_13a[key] = actual
        validations.append(validation_row(f"V18_13A_{key}", "PASS" if actual == expected else "FAIL", read_first_13a, expected, actual))

    no_token_ok, token_note = no_order_tokens_in_outputs(root)
    validations.append(validation_row("NO_ORDER_TOKEN_IN_V18_13A_OUTPUTS", "PASS" if no_token_ok else "FAIL", "outputs/v18/read_center/V18_13A*", "no immediate live trade or sell tokens", "clean" if no_token_ok else "found", token_note))
    ...
    report = f"""# V18.13A-R1 Stable Snapshot
...
- UNIFIED_READ_CENTER_STATUS: `{values_13a.get('UNIFIED_READ_CENTER_STATUS', '')}`
- OFFICIAL_DECISION_IMPACT: `{OFFICIAL_DECISION_IMPACT}`
- AUTO_SELL: `{AUTO_SELL}`
- AUTO_TRADE: `{AUTO_TRADE}`
- READ_LINK_ONLY: `{READ_LINK_ONLY}`
...
The snapshot contains only V18.13A unified daily read center link files, generated V18.13A outputs, README, manifest, validation checks, and restore tooling.
...
The V18.13A wrapper was run before copying. PowerShell parse checks, Python compile checks, critical file copy checks, official daily hash check, V18.12F-R2 and V18.12H-R1 stable snapshot manifest hash checks, and generated output token checks were recorded.
"""
    report_path.write_text(report, encoding="utf-8")

    read_first_out = f"""V18.13A-R1 STABLE SNAPSHOT READ FIRST
...
{values_13a.get('UNIFIED_READ_CENTER_STATUS', '')}
...
"""
    ...
    print(f"UNIFIED_READ_CENTER_STATUS: {values_13a.get('UNIFIED_READ_CENTER_STATUS', '')}")
    ...
    print(f"READ_LINK_ONLY: {READ_LINK_ONLY}")
