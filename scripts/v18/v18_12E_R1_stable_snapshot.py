...
MODE = "SNAPSHOT_ONLY"
STATUS_OK = "OK_V18_12E_R1_STABLE_SNAPSHOT_READY"
...
V18_12D_R1_SNAPSHOT_MODIFIED = "False"
SHADOW_ONLY = "True"

SNAPSHOT_FILES = [
    ("script", "scripts/v18/run_v18_12E_sell_timing_daily_wrapper.ps1"),
    ("script", "scripts/v18/v18_12E_sell_timing_read_center.py"),
    ("output", "outputs/v18/sell_timing/V18_12E_READ_FIRST.txt"),
    ("output", "outputs/v18/sell_timing/V18_12E_CURRENT_SELL_TIMING_DAILY_READ_CENTER.md"),
    ("output", "outputs/v18/sell_timing/V18_12E_CURRENT_SELL_TIMING_DAILY_SUMMARY.csv"),
    ("output", "outputs/v18/sell_timing/V18_12E_CURRENT_SELL_TIMING_DAILY_INPUT_AUDIT.csv"),
]

PS_PARSE_FILES = [
    "scripts/v18/run_v18_12E_sell_timing_daily_wrapper.ps1",
    "scripts/v18/run_v18_12E_R1_stable_snapshot.ps1",
]

PY_COMPILE_FILES = [
    "scripts/v18/v18_12E_sell_timing_read_center.py",
    "scripts/v18/v18_12E_R1_stable_snapshot.py",
]

V18_12E_OUTPUTS = [
    "outputs/v18/sell_timing/V18_12E_READ_FIRST.txt",
    "outputs/v18/sell_timing/V18_12E_CURRENT_SELL_TIMING_DAILY_READ_CENTER.md",
    "outputs/v18/sell_timing/V18_12E_CURRENT_SELL_TIMING_DAILY_SUMMARY.csv",
    "outputs/v18/sell_timing/V18_12E_CURRENT_SELL_TIMING_DAILY_INPUT_AUDIT.csv",
]
...
def run_v18_12e_wrapper(root: Path) -> Tuple[bool, str]:
    wrapper = root / "scripts/v18/run_v18_12E_sell_timing_daily_wrapper.ps1"
    try:
        proc = subprocess.run(
...
def no_order_tokens_in_outputs(root: Path) -> Tuple[bool, str]:
    hits: List[str] = []
    for rel_path in V18_12E_OUTPUTS:
        path = root / rel_path
...
Write-Host ""
Write-Host "=== V18.12E-R1 STABLE SNAPSHOT RESTORE ==="
Write-Host "SNAPSHOT: $Snapshot"
Write-Host "ROOT: $Root"
Write-Host "APPLY: $Apply"
Write-Host "NOTE: Restore covers only V18.12E sell timing daily wrapper/read center files."
...
def build_readme(snapshot: Path, manifest_path: Path, validation_path: Path, restore_path: Path) -> str:
    return f"""V18.12E-R1 STABLE SNAPSHOT README
...
Purpose:
Stable restore point for the V18.12E Sell Timing Daily Wrapper and Read Center after the A/B/C/D/E chain ran successfully.

Scope:
Copies the V18.12E daily wrapper, read center, READ_FIRST, read center Markdown, summary CSV, and input audit CSV.
...
V18_12D_R1_SNAPSHOT_MODIFIED={V18_12D_R1_SNAPSHOT_MODIFIED}

Restore:
The restore script is generated but not executed by snapshot creation. It restores only V18.12E sell timing daily wrapper/read center files.
...
def main() -> int:
    ...
    stamp = now_stamp()
    snapshot = root / "archive/stable" / f"V18_12E_R1_stable_sell_timing_daily_wrapper_{stamp}"
    ...
    manifest_path = snapshot / "V18_12E_R1_STABLE_MANIFEST.csv"
    validation_path = snapshot / "V18_12E_R1_STABLE_VALIDATION_CHECKS.csv"
    readme_path = snapshot / "V18_12E_R1_STABLE_SNAPSHOT_README.txt"
    restore_path = snapshot / "restore_v18_12E_R1_stable_snapshot.ps1"
    report_path = out_dir / "V18_12E_R1_CURRENT_STABLE_SNAPSHOT_REPORT.md"
    read_first_path = out_dir / "V18_12E_R1_READ_FIRST.txt"

    validations: List[Dict[str, str]] = []
    wrapper_ok, wrapper_output = run_v18_12e_wrapper(root)
    validations.append(validation_row("RUN_V18_12E_WRAPPER", "PASS" if wrapper_ok else "FAIL", root / "scripts/v18/run_v18_12E_sell_timing_daily_wrapper.ps1", "exit_0", "exit_0" if wrapper_ok else "nonzero", wrapper_output))
    ...
    read_first = root / "outputs/v18/sell_timing/V18_12E_READ_FIRST.txt"
    for key, expected in [
        ("STATUS", STATUS_OK.replace("_R1_STABLE_SNAPSHOT", "_SELL_TIMING_DAILY_WRAPPER")),
        ("OFFICIAL_DECISION_IMPACT", OFFICIAL_DECISION_IMPACT),
        ("AUTO_SELL", AUTO_SELL),
        ("AUTO_TRADE", AUTO_TRADE),
    ]:
        actual = read_first_value(read_first, key)
        validations.append(validation_row(f"V18_12E_{key}", "PASS" if actual == expected else "FAIL", read_first, expected, actual))

    no_token_ok, token_note = no_order_tokens_in_outputs(root)
    validations.append(validation_row("NO_ORDER_TOKEN_IN_V18_12E_OUTPUTS", "PASS" if no_token_ok else "FAIL", "outputs/v18/sell_timing/V18_12E*", "no order tokens", "clean" if no_token_ok else "found", token_note))
    ...
    status = STATUS_OK if fail_count == 0 else "WARN_V18_12E_R1_STABLE_SNAPSHOT_WITH_FAILURES"

    report = f"""# V18.12E-R1 Stable Snapshot
...
The snapshot contains only V18.12E sell timing daily wrapper/read center files, generated V18.12E outputs, README, manifest, validation checks, and restore tooling.
...
The V18.12E wrapper was run before copying. PowerShell parse checks, Python compile checks, critical file copy checks, official daily hash check, V18.12A/B/C/D stable snapshot manifest hash checks, and generated output order-token checks were recorded.
"""
    report_path.write_text(report, encoding="utf-8")

    read_first_out = f"""V18.12E-R1 STABLE SNAPSHOT READ FIRST
...
"""
    ...
    print(f"V18_12D_R1_SNAPSHOT_MODIFIED: {V18_12D_R1_SNAPSHOT_MODIFIED}")
