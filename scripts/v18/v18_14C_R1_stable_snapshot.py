...
STATUS_OK = "OK_V18_14C_R1_STABLE_SNAPSHOT_READY"
STATUS_WARN = "WARN_V18_14C_R1_STABLE_SNAPSHOT_WITH_FAILURES"
STATUS_FAIL = "FAIL_V18_14C_R1_STABLE_SNAPSHOT"

EXPECTED_CURRENT_STATUS = "OK_V18_14C_RANKED_CANDIDATE_FORWARD_TRACKER_READY"
...
SNAPSHOT_PREFIX = "V18_14C_R1_stable_ranked_candidate_forward_tracker"

SNAPSHOT_FILES = [
    ("script", "scripts/v18/run_v18_14C_ranked_candidate_forward_tracker.ps1", True),
    ("script", "scripts/v18/v18_14C_ranked_candidate_forward_tracker.py", True),
    ("script", "scripts/v18/run_v18_14C_R1_stable_snapshot.ps1", True),
    ("script", "scripts/v18/v18_14C_R1_stable_snapshot.py", True),
    ("state", "state/v18/candidate_forward_tracker/V18_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER.csv", True),
    ("v18_14c_output", "outputs/v18/candidates/V18_14C_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER.csv", True),
    ("v18_14c_output", "outputs/v18/candidates/V18_14C_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER_AUDIT.csv", True),
    ("v18_14c_output", "outputs/v18/ops/V18_14C_READ_FIRST.txt", True),
    ("v18_14c_output", "outputs/v18/ops/V18_14C_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER_REPORT.md", True),
    ("v18_14c_output", "outputs/v18/ops/V18_14C_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER_SUMMARY.csv", True),
    ("v18_14c_output", "outputs/v18/ops/V18_14C_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER_INPUT_AUDIT.csv", True),
    ("upstream_current", "outputs/v18/candidates/V18_CURRENT_RANKED_CANDIDATES.csv", False),
    ("upstream_current", "outputs/v18/read_center/V18_CURRENT_READ_FIRST.txt", False),
    ("upstream_current", "outputs/v18/ops/V18_14B_READ_FIRST.txt", False),
    ("upstream_current", "outputs/v18/ops/V18_14B_R1_READ_FIRST.txt", False),
]

PS_PARSE_FILES = [
    "scripts/v18/run_v18_14C_ranked_candidate_forward_tracker.ps1",
    "scripts/v18/run_v18_14C_R1_stable_snapshot.ps1",
]

PY_COMPILE_FILES = [
    "scripts/v18/v18_14C_ranked_candidate_forward_tracker.py",
    "scripts/v18/v18_14C_R1_stable_snapshot.py",
]
...
def build_restore_script(snapshot: Path) -> str:
    restore_files = [
        ...
        if category in {"script", "state", "v18_14c_output"} and required
    ]
...
Write-Host "=== V18.14C-R1 STABLE SNAPSHOT RESTORE ==="
...
if (-not $Apply) {{
    Write-Host "DRYRUN_ONLY. Re-run with -Apply to restore V18.14C-R1 forward tracker files."
}}
else {{
...
def build(root: Path) -> Tuple[Dict[str, str], int]:
    ...
    readme_path = snapshot / "README_V18_14C_R1_STABLE_SNAPSHOT.md"
    restore_path = snapshot / "RESTORE_V18_14C_R1.ps1"
    report_path = ops_dir / "V18_14C_R1_CURRENT_STABLE_SNAPSHOT_REPORT.md"
    read_first_path = ops_dir / "V18_14C_R1_READ_FIRST.txt"

    current_read_first = root / "outputs/v18/ops/V18_14C_READ_FIRST.txt"
    ...
    for key, expected_value in expected.items():
        ...
        validations.append(validation_row(f"CURRENT_V18_14C_{key}", "PASS" if actual == expected_value else "FAIL", current_read_first, expected_value, actual))
    ...
    readme = f"""# V18.14C-R1 Stable Snapshot

Created: {now_text()}

Snapshot: {snapshot}

Purpose: Stable restore point for the V18.14C ranked candidate forward tracker.

Scope: Snapshot only. This does not modify V18.13D, V18.14A/B, official daily, or existing stable snapshots.
...
Interpretation: PENDING_FORWARD_ROWS is expected for new 2026-05-18 signals until later local prices are available.
...
"""
    ...
    scan_paths = [
        ...
        root / "outputs/v18/candidates/V18_14C_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER_AUDIT.csv",
        root / "outputs/v18/ops/V18_14C_READ_FIRST.txt",
        root / "outputs/v18/ops/V18_14C_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER_REPORT.md",
        root / "outputs/v18/ops/V18_14C_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER_SUMMARY.csv",
        root / "outputs/v18/ops/V18_14C_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER_INPUT_AUDIT.csv",
        ...
        snapshot / "outputs/v18/candidates/V18_14C_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER_AUDIT.csv",
        snapshot / "outputs/v18/ops/V18_14C_READ_FIRST.txt",
        snapshot / "outputs/v18/ops/V18_14C_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER_REPORT.md",
        snapshot / "outputs/v18/ops/V18_14C_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER_SUMMARY.csv",
        snapshot / "outputs/v18/ops/V18_14C_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER_INPUT_AUDIT.csv",
    ...
    validations.append(validation_row("NO_DANGEROUS_TOKEN_IN_V18_14C_OUTPUTS_AND_SNAPSHOT", "PASS" if not hits else "FAIL", "V18.14C outputs and snapshot artifacts", "clean", "clean" if not hits else "found", "NONE" if not hits else ";".join(hits[:10])))
    ...
    read_first_keys = [
        ...
        "VALIDATION_FAIL_COUNT", "TRACKER_ROWS", "NEW_SIGNAL_ROWS_ADDED", "FORWARD_COMPLETE_ROWS",
        "PENDING_FORWARD_ROWS", "TOP_5_TICKERS", "OFFICIAL_DECISION_IMPACT", "AUTO_TRADE", "AUTO_SELL",
        "SNAPSHOT_ONLY", "DANGEROUS_TOKEN_DETECTED", "DUPLICATE_PREVENTION_VALIDATED",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create V18.14C-R1 ranked candidate forward tracker stable snapshot.")
