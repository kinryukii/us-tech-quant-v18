...
STATUS_OK = "OK_V18_14E_R1_STABLE_SNAPSHOT_READY"
STATUS_WARN = "WARN_V18_14E_R1_STABLE_SNAPSHOT_WITH_FAILURES"
STATUS_FAIL = "FAIL_V18_14E_R1_STABLE_SNAPSHOT"

EXPECTED_CURRENT_STATUS = "OK_V18_14E_CURRENT_DAILY_WITH_FORWARD_TRACKER_READY"
EXPECTED_RUN_MODE = "READ_CENTER_REFRESH_ONLY"
EXPECTED_FORWARD_TRACKER_RUN = "RAN"
EXPECTED_14B_STATUS = "OK_V18_14B_CURRENT_DAILY_COMMAND_CENTER_REFRESH_READY"
EXPECTED_14C_STATUS = "OK_V18_14C_RANKED_CANDIDATE_FORWARD_TRACKER_READY"
EXPECTED_14D_STATUS = "OK_V18_14D_RANKED_CANDIDATE_FORWARD_PRICE_FILLER_READY"
EXPECTED_TRACKER_ROWS = "20"
EXPECTED_PENDING_FORWARD_ROWS = "20"
EXPECTED_PRICE_SOURCE_STATUS = "LOCAL_PRICE_AVAILABLE"
...
SNAPSHOT_PREFIX = "V18_14E_R1_stable_current_daily_with_forward_tracker"

SNAPSHOT_FILES = [
    ("script", "scripts/v18/run_v18_14E_current_daily_with_forward_tracker.ps1", True),
    ("script", "scripts/v18/v18_14E_current_daily_with_forward_tracker.py", True),
    ("script", "scripts/v18/run_v18_current_daily_command_center.ps1", True),
    ("script", "scripts/v18/run_v18_14E_R1_stable_snapshot.ps1", True),
    ("script", "scripts/v18/v18_14E_R1_stable_snapshot.py", True),
    ("v18_14e_output", "outputs/v18/ops/V18_14E_READ_FIRST.txt", True),
    ("v18_14e_output", "outputs/v18/ops/V18_14E_CURRENT_DAILY_WITH_FORWARD_TRACKER_REPORT.md", True),
    ("v18_14e_output", "outputs/v18/ops/V18_14E_CURRENT_DAILY_WITH_FORWARD_TRACKER_SUMMARY.csv", True),
    ("v18_14e_output", "outputs/v18/ops/V18_14E_CURRENT_DAILY_WITH_FORWARD_TRACKER_INPUT_AUDIT.csv", True),
    ("current_alias", "outputs/v18/candidates/V18_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER.csv", True),
    ("current_alias", "outputs/v18/ops/V18_CURRENT_FORWARD_TRACKER_READ_FIRST.txt", True),
    ("current_alias", "outputs/v18/ops/V18_CURRENT_FORWARD_PRICE_FILLER_READ_FIRST.txt", True),
    ("upstream", "scripts/v18/run_v18_14B_current_daily_command_center.ps1", False),
    ("upstream", "scripts/v18/v18_14B_current_daily_command_center.py", False),
    ("upstream", "scripts/v18/run_v18_14C_ranked_candidate_forward_tracker.ps1", False),
    ("upstream", "scripts/v18/v18_14C_ranked_candidate_forward_tracker.py", False),
    ("upstream", "scripts/v18/run_v18_14D_ranked_candidate_forward_price_filler.ps1", False),
    ("upstream", "scripts/v18/v18_14D_ranked_candidate_forward_price_filler.py", False),
    ("upstream", "outputs/v18/ops/V18_14B_READ_FIRST.txt", False),
    ("upstream", "outputs/v18/ops/V18_14C_READ_FIRST.txt", False),
    ("upstream", "outputs/v18/ops/V18_14D_READ_FIRST.txt", False),
...
PS_PARSE_FILES = [
    ...
    "scripts/v18/run_v18_14E_R1_stable_snapshot.ps1",
]

PY_COMPILE_FILES = [
    "scripts/v18/v18_14E_current_daily_with_forward_tracker.py",
    "scripts/v18/v18_14E_R1_stable_snapshot.py",
]
...
def duplicate_key_count(rows: Sequence[Dict[str, str]]) -> int:
...
def build_restore_script(snapshot: Path) -> str:
    restore_files = [
        ...
        if category in {"script", "v18_14e_output", "current_alias"} and required
    ]
...
Write-Host "=== V18.14E-R1 STABLE SNAPSHOT RESTORE ==="
...
if (-not $Apply) {{
    Write-Host "DRYRUN_ONLY. Re-run with -Apply to restore V18.14E-R1 current entry files."
}}
else {{
...
def build(root: Path) -> Tuple[Dict[str, str], int]:
    ...
    readme_path = snapshot / "README_V18_14E_R1_STABLE_SNAPSHOT.md"
    restore_path = snapshot / "RESTORE_V18_14E_R1.ps1"
    report_path = ops_dir / "V18_14E_R1_CURRENT_STABLE_SNAPSHOT_REPORT.md"
    read_first_path = ops_dir / "V18_14E_R1_READ_FIRST.txt"

    current_read_first = root / "outputs/v18/ops/V18_14E_READ_FIRST.txt"
    tracker_alias = root / "outputs/v18/candidates/V18_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER.csv"
    tracker_rows = read_csv_rows(tracker_alias)
    duplicate_count = duplicate_key_count(tracker_rows)

    current_values = {
        "STATUS": first_value(current_read_first, "STATUS"),
        "RUN_MODE": first_value(current_read_first, "RUN_MODE"),
        "FORWARD_TRACKER_RUN": first_value(current_read_first, "FORWARD_TRACKER_RUN"),
        "V18_14B_STATUS": first_value(current_read_first, "V18_14B_STATUS"),
        "V18_14C_STATUS": first_value(current_read_first, "V18_14C_STATUS"),
        "V18_14D_STATUS": first_value(current_read_first, "V18_14D_STATUS"),
        "TRACKER_ROWS": first_value(current_read_first, "TRACKER_ROWS"),
        "NEW_SIGNAL_ROWS_ADDED": first_value(current_read_first, "NEW_SIGNAL_ROWS_ADDED"),
        "UPDATED_FORWARD_ROWS": first_value(current_read_first, "UPDATED_FORWARD_ROWS"),
        "FORWARD_COMPLETE_ROWS": first_value(current_read_first, "FORWARD_COMPLETE_ROWS"),
        "PENDING_FORWARD_ROWS": first_value(current_read_first, "PENDING_FORWARD_ROWS"),
        "PRICE_SOURCE_STATUS": first_value(current_read_first, "PRICE_SOURCE_STATUS"),
    ...
    expected = {
        ...
        "FORWARD_TRACKER_RUN": EXPECTED_FORWARD_TRACKER_RUN,
        "V18_14B_STATUS": EXPECTED_14B_STATUS,
        "V18_14C_STATUS": EXPECTED_14C_STATUS,
        "V18_14D_STATUS": EXPECTED_14D_STATUS,
        "TRACKER_ROWS": EXPECTED_TRACKER_ROWS,
        "PENDING_FORWARD_ROWS": EXPECTED_PENDING_FORWARD_ROWS,
        "PRICE_SOURCE_STATUS": EXPECTED_PRICE_SOURCE_STATUS,
    ...
    for key, expected_value in expected.items():
        ...
        validations.append(validation_row(f"CURRENT_V18_14E_{key}", "PASS" if actual == expected_value else "FAIL", current_read_first, expected_value, actual))

    alias_files = [
        root / "outputs/v18/candidates/V18_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER.csv",
        root / "outputs/v18/ops/V18_CURRENT_FORWARD_TRACKER_READ_FIRST.txt",
        root / "outputs/v18/ops/V18_CURRENT_FORWARD_PRICE_FILLER_READ_FIRST.txt",
    ]
    for alias in alias_files:
        if alias.suffix.lower() == ".csv":
            rows = read_csv_rows(alias)
            ok = bool(rows)
            actual = f"rows={len(rows)}" if ok else "missing_or_empty"
        else:
            ok = alias.exists() and len(read_text(alias)) > 0
            actual = "readable" if ok else "missing_or_empty"
        validations.append(validation_row(f"CURRENT_FORWARD_ALIAS_READABLE_{alias.name}", "PASS" if ok else "FAIL", alias, "readable", actual))
    validations.append(validation_row("TRACKER_DEDUP_SIGNAL_DATE_TICKER_RANK_SOURCE", "PASS" if duplicate_count == 0 else "FAIL", tracker_alias, "0", str(duplicate_count)))
    ...
    readme = f"""# V18.14E-R1 Stable Snapshot

Created: {now_text()}

Snapshot: {snapshot}

Purpose: Stable restore point for V18.14E current daily command center with forward tracker integration.

Accepted state:
STATUS: {current_values['STATUS']}
RUN_MODE: {current_values['RUN_MODE']}
FORWARD_TRACKER_RUN: {current_values['FORWARD_TRACKER_RUN']}
V18_14B_STATUS: {current_values['V18_14B_STATUS']}
V18_14C_STATUS: {current_values['V18_14C_STATUS']}
V18_14D_STATUS: {current_values['V18_14D_STATUS']}
TRACKER_ROWS: {current_values['TRACKER_ROWS']}
NEW_SIGNAL_ROWS_ADDED: {current_values['NEW_SIGNAL_ROWS_ADDED']}
UPDATED_FORWARD_ROWS: {current_values['UPDATED_FORWARD_ROWS']}
FORWARD_COMPLETE_ROWS: {current_values['FORWARD_COMPLETE_ROWS']}
PENDING_FORWARD_ROWS: {current_values['PENDING_FORWARD_ROWS']}
PRICE_SOURCE_STATUS: {current_values['PRICE_SOURCE_STATUS']}
...
"""
    ...
    scan_paths = [
        root / "outputs/v18/ops/V18_14E_READ_FIRST.txt",
        root / "outputs/v18/ops/V18_14E_CURRENT_DAILY_WITH_FORWARD_TRACKER_REPORT.md",
        root / "outputs/v18/ops/V18_14E_CURRENT_DAILY_WITH_FORWARD_TRACKER_SUMMARY.csv",
        root / "outputs/v18/ops/V18_14E_CURRENT_DAILY_WITH_FORWARD_TRACKER_INPUT_AUDIT.csv",
        root / "outputs/v18/candidates/V18_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER.csv",
        root / "outputs/v18/ops/V18_CURRENT_FORWARD_TRACKER_READ_FIRST.txt",
        root / "outputs/v18/ops/V18_CURRENT_FORWARD_PRICE_FILLER_READ_FIRST.txt",
        snapshot / "outputs/v18/ops/V18_14E_READ_FIRST.txt",
        snapshot / "outputs/v18/ops/V18_14E_CURRENT_DAILY_WITH_FORWARD_TRACKER_REPORT.md",
        snapshot / "outputs/v18/ops/V18_14E_CURRENT_DAILY_WITH_FORWARD_TRACKER_SUMMARY.csv",
        snapshot / "outputs/v18/ops/V18_14E_CURRENT_DAILY_WITH_FORWARD_TRACKER_INPUT_AUDIT.csv",
        snapshot / "outputs/v18/candidates/V18_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER.csv",
        snapshot / "outputs/v18/ops/V18_CURRENT_FORWARD_TRACKER_READ_FIRST.txt",
        snapshot / "outputs/v18/ops/V18_CURRENT_FORWARD_PRICE_FILLER_READ_FIRST.txt",
        snapshot / "outputs/v18/ops/V18_14B_READ_FIRST.txt",
        snapshot / "outputs/v18/ops/V18_14C_READ_FIRST.txt",
        snapshot / "outputs/v18/ops/V18_14D_READ_FIRST.txt",
        snapshot / "state/v18/candidate_forward_tracker/V18_CURRENT_RANKED_CANDIDATE_FORWARD_TRACKER.csv",
        readme_path,
        restore_path,
    ]
    hits = dangerous_hits(root, scan_paths)
    validations.append(validation_row("NO_DANGEROUS_TOKEN_IN_V18_14E_OUTPUTS_AND_SNAPSHOT", "PASS" if not hits else "FAIL", "V18.14E outputs and snapshot artifacts", "clean", "clean" if not hits else "found", "NONE" if not hits else ";".join(hits[:10])))
    ...
    values = {
        ...
        "DUPLICATE_PREVENTION_VALIDATED": "YES" if duplicate_count == 0 else "NO",
    ...
    read_first_keys = [
        ...
        "VALIDATION_FAIL_COUNT", "RUN_MODE", "FORWARD_TRACKER_RUN", "V18_14B_STATUS", "V18_14C_STATUS",
        "V18_14D_STATUS", "TRACKER_ROWS", "NEW_SIGNAL_ROWS_ADDED", "UPDATED_FORWARD_ROWS",
        "FORWARD_COMPLETE_ROWS", "PENDING_FORWARD_ROWS", "PRICE_SOURCE_STATUS", "TOP_5_TICKERS",
        "OFFICIAL_DECISION_IMPACT", "AUTO_TRADE", "AUTO_SELL", "SNAPSHOT_ONLY",
        "DANGEROUS_TOKEN_DETECTED", "DUPLICATE_PREVENTION_VALIDATED", "PROTECTED_STABLE_SNAPSHOTS_UNCHANGED",
    ]
    write_text(read_first_path, "\n".join(f"{key}: {values[key]}" for key in read_first_keys) + "\n")

    report = [
        "# V18.14E-R1 Stable Snapshot",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create V18.14E-R1 current daily with forward tracker stable snapshot.")
