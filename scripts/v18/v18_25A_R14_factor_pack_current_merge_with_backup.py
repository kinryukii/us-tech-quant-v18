...
STATUS_OK = "OK_V18_25A_R14_FACTOR_PACK_CURRENT_MERGE_READY"
STATUS_WARN = "WARN_V18_25A_R14_FACTOR_PACK_CURRENT_MERGE_READY"
STATUS_FAIL = "FAIL_V18_25A_R14_FACTOR_PACK_CURRENT_MERGE"
MODE = "FACTOR_PACK_CURRENT_MERGE_WITH_BACKUP"

R13_READ_FIRST = "outputs/v18/ops/V18_25A_R13_READ_FIRST.txt"
R13_STAGED = "outputs/v18/factor_pack/V18_25A_R13_TARGETED_FACTOR_PACK_STAGED.csv"
R13_GATE = "outputs/v18/factor_pack/V18_25A_R13_TARGETED_FACTOR_PACK_MERGE_GATE.csv"
R13_SCHEMA = "outputs/v18/factor_pack/V18_25A_R13_TARGETED_FACTOR_PACK_SCHEMA_AUDIT.csv"
CURRENT_FACTOR = "outputs/v18/factor_pack/V18_CURRENT_RAW105_FACTOR_PACK_RANKING.csv"
BACKUP_ROOT = "archive/v18/factor_pack_backups"

OUT_PLAN = "outputs/v18/factor_pack/V18_25A_R14_CURRENT_FACTOR_PACK_MERGE_PLAN.csv"
OUT_RESULT = "outputs/v18/factor_pack/V18_25A_R14_CURRENT_FACTOR_PACK_MERGE_RESULT.csv"
OUT_SCHEMA = "outputs/v18/factor_pack/V18_25A_R14_CURRENT_FACTOR_PACK_SCHEMA_MERGE_AUDIT.csv"
OUT_VALIDATION = "outputs/v18/factor_pack/V18_25A_R14_CURRENT_FACTOR_PACK_POST_MERGE_VALIDATION.csv"
OUT_REPORT = "outputs/v18/degraded_daily_review/V18_25A_R14_CURRENT_REPORT.md"
OUT_READ_FIRST = "outputs/v18/ops/V18_25A_R14_READ_FIRST.txt"
OUT_OPS_REPORT = "outputs/v18/ops/V18_25A_R14_CURRENT_FACTOR_PACK_MERGE_REPORT.md"
...
READ_FIRST_FIELDS = [
    ...
    "BACKUP_DIR",
    "RESTORE_SCRIPT_PATH",
    "R13_MERGE_READY_COUNT",
    "R13_MERGE_BLOCKED_COUNT",
    "TARGET_TICKER_COUNT",
...
def allowed_r14_output(path: Path, root: Path) -> bool:
...
def protected_files(root: Path) -> List[Path]:
    ...
    for rel in rels:
        ...
        for path in files:
            try:
                if allowed_r14_output(path, root):
                    continue
            except ValueError:
                pass
            out.append(path)
...
Write-Host "===== RESTORE V18.25A-R14 FACTOR PACK START ====="
...
Write-Host "===== RESTORE V18.25A-R14 FACTOR PACK END ====="
"""
...
def recompute_rank(rows: List[Dict[str, object]], fields: Sequence[str]) -> Tuple[List[Dict[str, object]], str]:
    if "factor_pack_rank" not in fields or "factor_pack_score" not in fields or "ticker" not in fields:
        return rows, "RANK_RECOMPUTE_SKIPPED_AMBIGUOUS"
    ordered = sorted(rows, key=lambda row: (-to_float(row.get("factor_pack_score")), norm_ticker(row.get("ticker"))))
    for idx, row in enumerate(ordered, start=1):
        row["factor_pack_rank"] = idx
    return ordered, "RANK_RECOMPUTED_BY_SCORE_DESC_TICKER_ASC"
...
def render_report(values: Dict[str, str]) -> str:
    return f"""# V18.25A-R14 Factor Pack Current Merge With Backup
...
Current factor pack: `{values['CURRENT_FACTOR_PACK_PATH']}`

Staged factor path: `{values['STAGED_FACTOR_PATH']}`
...
Rank recompute: {values['RANK_RECOMPUTE_STATUS']}

Safety: no external fetch, no price cache/market proxy/technical/tier/degraded-current/decision/trading permission changes. OFFICIAL_DECISION_IMPACT remains NONE.
...
def main() -> int:
    ...
    script_path = root / "scripts/v18/v18_25A_R14_factor_pack_current_merge_with_backup.py"
    wrapper_path = root / "scripts/v18/run_v18_25A_R14_factor_pack_current_merge_with_backup.ps1"
    current_path = root / CURRENT_FACTOR
    staged_path = root / R13_STAGED
    timestamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_dir = root / BACKUP_ROOT / f"V18_25A_R14_{timestamp}"
    backup_file = backup_dir / "V18_CURRENT_RAW105_FACTOR_PACK_RANKING.before_V18_25A_R14.csv"
    restore_path = backup_dir / "RESTORE_V18_25A_R14_FACTOR_PACK.ps1"
    ...
    r13 = parse_read_first(root / R13_READ_FIRST)
    gate_rows, _ = read_csv(root / R13_GATE)
    schema_rows, _ = read_csv(root / R13_SCHEMA)
    ...
    preconditions = {
        "r13_status_ok": r13.get("STATUS", "").startswith("OK_"),
        "r13_merge_ready_52": r13.get("MERGE_READY_COUNT", "") == "52",
        "r13_merge_blocked_0": r13.get("MERGE_BLOCKED_COUNT", "") == "0",
        ...
        "formula_compatible": r13.get("FACTOR_FORMULA_COMPATIBILITY_STATUS", "") in {"CURRENT_FACTOR_COMPATIBLE_LOCAL_REIMPLEMENTATION", "EXACT_COMPATIBLE", "EXACT_CURRENT_FACTOR_FORMULA_REUSED"},
        "vix_included": r13.get("VIX_INCLUDED", "").upper() == "TRUE",
    }
    ...
    backup_rows = [
        {"backup_item": "backup_dir", "path": str(backup_dir), "status": str(backup_dir.exists()).upper(), "notes": "Timestamped R14 factor pack backup directory."},
        {"backup_item": "current_factor_backup", "path": str(backup_file), "status": str(backup_file.exists()).upper(), "notes": "Original current factor pack before merge."},
        {"backup_item": "restore_script", "path": str(restore_path), "status": str(restore_path.exists()).upper(), "notes": "Restores original V18_CURRENT_RAW105_FACTOR_PACK_RANKING.csv."},
    ]
    ...
    rank_status = "RANK_RECOMPUTE_NOT_ATTEMPTED"
    ...
    if all(preconditions.values()) and backup_file.exists() and restore_path.exists():
        ...
        post_rows, rank_status = recompute_rank(post_rows, current_fields)
    ...
    comparison_fields = [field for field in current_fields if field != "factor_pack_rank"]
    ...
    for ticker in target_tickers:
        ...
        if row_fingerprint(rows_after[0], comparison_fields) == row_fingerprint(expected, comparison_fields):
    ...
    validations = [
        ...
        validation_row("target_rows_match_staged_aligned_except_rank", aligned_match_count == len(target_tickers), len(target_tickers) - aligned_match_count, str(aligned_match_count)),
        ...
        validation_row("no_external_fetch", True, 0, "R14 does not fetch external data."),
    ...
    status = STATUS_FAIL
    if fail_count == 0 and merge_fail == 0 and schema_status == "MERGE_SCHEMA_COMPATIBLE" and rank_status.startswith("RANK_RECOMPUTED"):
        status = STATUS_OK
    ...
    values = {
        "STATUS": status,
        "MODE": MODE,
        "R13_SOURCE_PATH": str(root / R13_READ_FIRST),
        "CURRENT_FACTOR_PACK_PATH": str(current_path),
        "STAGED_FACTOR_PATH": str(staged_path),
        "BACKUP_DIR": str(backup_dir),
        "RESTORE_SCRIPT_PATH": str(restore_path),
        "R13_MERGE_READY_COUNT": r13.get("MERGE_READY_COUNT", str(gate_ready)),
        "R13_MERGE_BLOCKED_COUNT": r13.get("MERGE_BLOCKED_COUNT", str(gate_blocked)),
        ...
        "RANK_RECOMPUTE_STATUS": rank_status,
        "CURRENT_FACTOR_PACK_MODIFIED": str(current_modified).upper(),
        "FACTOR_PACK_MODIFIED": str(current_modified).upper(),
        ...
        "NEXT_RECOMMENDED_STEP": "Run a post-merge R6/R7 promotion blocker audit to verify BLOCKED_MISSING_FACTOR_SCORE is cleared.",
    }

    plan_rows = [
        {"plan_item": "backup_current_factor_pack", "path": str(backup_file), "status": str(backup_file.exists()).upper(), "notes": "Backup created before current factor pack modification."},
        {"plan_item": "restore_script", "path": str(restore_path), "status": str(restore_path.exists()).upper(), "notes": "Restores pre-R14 current factor pack ranking."},
        {"plan_item": "merge_strategy", "path": str(current_path), "status": "REPLACE_OR_APPEND_BY_TICKER", "notes": "Preserve schema and non-target rows."},
        {"plan_item": "rank_recompute", "path": str(current_path), "status": rank_status, "notes": "Recompute rank by factor_pack_score desc and ticker asc when ranking columns exist."},
    ]
    result_rows = [
        ...
        {"merge_item": "target_rows_match_staged_aligned_except_rank", "value": aligned_match_count, "notes": "Rank differs by deterministic full-file recompute."},
    ]
    schema_audit_rows = [
        ...
        {"schema_item": "schema_compatibility_status", "value": schema_status, "notes": "Current factor pack schema preserved."},
        {"schema_item": "r13_schema_compatibility_status", "value": schema_lookup.get("compatibility_status", ""), "notes": str(root / R13_SCHEMA)},
    ]
