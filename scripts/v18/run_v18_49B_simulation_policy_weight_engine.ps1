...
$Script = Join-Path $ProjectRoot "scripts\v18\v18_49B_simulation_policy_weight_engine.py"
$ReadFirst = Join-Path $ProjectRoot "outputs\v18\ops\V18_49B_READ_FIRST.txt"

Write-Host "DELEGATING_TO: $Script"
Write-Host "PATCH_VERSION: V18.49B-R1"
Write-Host "PATCH_NAME: OPTIONS_PROMOTION_GATE_AND_HISTORY_NORMALIZATION_REPAIR"
...
$Args49B = @("--project-root", $ProjectRoot)
if ($WriteCurrent) {
    $Args49B += "--write-current"
}
if ($VerboseLog) {
    Write-Host "PYTHON: $Python"
    Write-Host "ARGS: $($Args49B -join ' ')"
}

& $Python $Script @Args49B
$ExitCode = $LASTEXITCODE

Write-Host "SCRIPT_EXIT_CODE: $ExitCode"
Write-Host "READ_FIRST_PATH: $ReadFirst"
if (Test-Path $ReadFirst) {
    Write-Host "--- V18.49B READ_FIRST ---"
...
if ($Strict -and (Test-Path $ReadFirst)) {
    ...
    if ($StatusLine -like "STATUS: WARN_V18_49B_SOURCE_BACKTEST_LOW_EVIDENCE*" -or
        $StatusLine -like "STATUS: WARN_V18_49B_SOURCE_BACKTEST_MISSING*") {
        exit 2
    }
}
