...
STATUS_OK = "OK_V18_13C_R1_STABLE_SNAPSHOT_READY"
STATUS_WARN = "WARN_V18_13C_R1_STABLE_SNAPSHOT_WITH_FAILURES"
STATUS_FAIL_DANGEROUS = "FAIL_V18_13C_R1_DANGEROUS_TOKEN_FOUND"

EXPECTED_STATUS = "OK_V18_13C_UNIFIED_DAILY_WITH_RANKED_CANDIDATES_READY"
...
SNAPSHOT_NAME_PREFIX = "V18_13C_R1_stable_unified_daily_with_ranked_candidates"

SNAPSHOT_FILES = [
    ("script", "scripts/v18/v18_13C_ranked_candidate_unified_link.py"),
    ("script", "scripts/v18/run_v18_13C_ranked_candidate_unified_link.ps1"),
    ("script", "scripts/v18/v18_13C_R1_stable_snapshot.py"),
    ("script", "scripts/v18/run_v18_13C_R1_stable_snapshot.ps1"),
    ("output", "outputs/v18/read_center/V18_13C_CURRENT_UNIFIED_DAILY_WITH_RANKED_CANDIDATES.md"),
    ("output", "outputs/v18/read_center/V18_13C_READ_FIRST.txt"),
    ("output", "outputs/v18/read_center/V18_13C_CURRENT_UNIFIED_DAILY_WITH_RANKED_CANDIDATES_SUMMARY.csv"),
    ("output", "outputs/v18/read_center/V18_13C_CURRENT_UNIFIED_DAILY_WITH_RANKED_CANDIDATES_INPUT_AUDIT.csv"),
    ("linked_source", "outputs/v18/read_center/V18_13A_READ_FIRST.txt"),
    ("linked_source", "outputs/v18/read_center/V18_13A_CURRENT_UNIFIED_DAILY_READ_CENTER.md"),
    ("linked_source", "outputs/v18/read_center/V18_13B_READ_FIRST.txt"),
    ("linked_source", "outputs/v18/read_center/V18_13B_CURRENT_RANKED_CANDIDATE_READ_CENTER.md"),
    ("linked_source", "outputs/v18/candidates/V18_13B_CURRENT_RANKED_CANDIDATES.csv"),
    ("linked_source", "outputs/v18/candidates/V18_13B_CURRENT_RANKED_CANDIDATES_INPUT_AUDIT.csv"),
]

PS_PARSE_FILES = [
    "scripts/v18/run_v18_13C_R1_stable_snapshot.ps1",
    "scripts/v18/run_v18_13C_ranked_candidate_unified_link.ps1",
]

PY_COMPILE_FILES = [
    "scripts/v18/v18_13C_R1_stable_snapshot.py",
    "scripts/v18/v18_13C_ranked_candidate_unified_link.py",
]
...
TOKEN_SCAN_OUTPUTS = [
    "outputs/v18/read_center/V18_13C_CURRENT_UNIFIED_DAILY_WITH_RANKED_CANDIDATES.md",
    "outputs/v18/read_center/V18_13C_READ_FIRST.txt",
    "outputs/v18/read_center/V18_13C_CURRENT_UNIFIED_DAILY_WITH_RANKED_CANDIDATES_SUMMARY.csv",
    "outputs/v18/read_center/V18_13C_CURRENT_UNIFIED_DAILY_WITH_RANKED_CANDIDATES_INPUT_AUDIT.csv",
    "outputs/v18/ops/V18_13C_R1_CURRENT_STABLE_SNAPSHOT_REPORT.md",
    "outputs/v18/ops/V18_13C_R1_READ_FIRST.txt",
]
...
def run_v18_13c(root: Path) -> Tuple[bool, str]:
    wrapper = root / "scripts/v18/run_v18_13C_ranked_candidate_unified_link.ps1"
    try:
        proc = subprocess.run(
...
Write-Host ""
Write-Host "=== V18.13C-R1 STABLE SNAPSHOT RESTORE ==="
Write-Host "SNAPSHOT: $Snapshot"
Write-Host "ROOT: $Root"
Write-Host "APPLY: $Apply"
Write-Host "NOTE: Restore covers only V18.13C link scripts, copied linked-source outputs, and V18.13C read-center outputs."
...
def build_readme(snapshot: Path, manifest_path: Path, validation_path: Path, restore_path: Path) -> str:
    return f"""# V18.13C-R1 Stable Snapshot

Created: {now_text()}

Snapshot: {snapshot}

Purpose: Stable restore point for V18.13C Unified Daily With Ranked Candidates.

Scope: Copies the V18.13C link scripts and outputs, plus available V18.13A/V18.13B linked source outputs, manifest, validation, README, and restore tooling.
...
Restore: RESTORE_V18_13C_R1.ps1 is generated but not executed by snapshot creation. It restores only files listed in MANIFEST.csv.
...
def main() -> int:
    ...
    readme_path = snapshot / "README_V18_13C_R1_STABLE_SNAPSHOT.md"
    restore_path = snapshot / "RESTORE_V18_13C_R1.ps1"
    report_path = out_dir / "V18_13C_R1_CURRENT_STABLE_SNAPSHOT_REPORT.md"
    read_first_path = out_dir / "V18_13C_R1_READ_FIRST.txt"

    c_summary_path = root / "outputs/v18/read_center/V18_13C_CURRENT_UNIFIED_DAILY_WITH_RANKED_CANDIDATES_SUMMARY.csv"
    c_read_center_path = root / "outputs/v18/read_center/V18_13C_CURRENT_UNIFIED_DAILY_WITH_RANKED_CANDIDATES.md"
    b_read_center_path = root / "outputs/v18/read_center/V18_13B_CURRENT_RANKED_CANDIDATE_READ_CENTER.md"

    validations: List[Dict[str, str]] = []
    wrapper_ok, wrapper_output = run_v18_13c(root)
    validations.append(validation_row("RUN_V18_13C_RANKED_CANDIDATE_UNIFIED_LINK", "PASS" if wrapper_ok else "FAIL", root / "scripts/v18/run_v18_13C_ranked_candidate_unified_link.ps1", "exit_0", "exit_0" if wrapper_ok else "nonzero", wrapper_output))

    summary = summary_values(c_summary_path)
    c_text = read_text(c_read_center_path)
    b_text = read_text(b_read_center_path)
    ranking_source_policy = read_first_value(c_read_center_path, "ranking_source_policy") or read_first_value(b_read_center_path, "ranking_source_policy")
    fallback_used = read_first_value(c_read_center_path, "fallback_used") or read_first_value(b_read_center_path, "fallback_used")
    selected_for_ranking_count = read_first_value(b_read_center_path, "selected_for_ranking_count")
    ...
    values = {
        ...
        "LINK_ONLY": summary.get("LINK_ONLY", ""),
        "SNAPSHOT_ONLY": SNAPSHOT_ONLY,
        "ranking_source_policy": ranking_source_policy,
        "fallback_used": fallback_used,
        "selected_for_ranking_count": selected_for_ranking_count,
    ...
    for key, expected in expected_values.items():
        path = c_summary_path if key not in {"ranking_source_policy", "fallback_used", "selected_for_ranking_count"} else c_read_center_path
        if key == "selected_for_ranking_count":
            path = b_read_center_path
        append_expectation(validations, f"V18_13C_{key}", path, expected, values.get(key, ""))

    validations.append(validation_row("V18_13C_SOURCE_DEDUP_POLICY_PRESENT", "PASS" if "## SOURCE_DEDUP_POLICY" in c_text else "FAIL", c_read_center_path, "present", "present" if "## SOURCE_DEDUP_POLICY" in c_text else "missing"))
    validations.append(validation_row("V18_13B_SOURCE_DEDUP_POLICY_PRESENT", "PASS" if "selected_for_ranking_count: 2" in b_text else "FAIL", b_read_center_path, "present", "present" if "selected_for_ranking_count: 2" in b_text else "missing"))
    ...
    report = f"""# V18.13C-R1 Stable Snapshot
...
"""
    ...
    read_first_out = f"""V18.13C-R1 STABLE SNAPSHOT READ FIRST
...
"""
    ...
    validations.append(validation_row("NO_DANGEROUS_TOKEN_IN_V18_13C_AND_R1_OUTPUTS", "PASS" if not hits else "FAIL", "V18.13C outputs and V18.13C-R1 snapshot outputs", "clean", "clean" if not hits else "found", "clean" if not hits else "redacted_hits_present"))
    if hits:
