...
def build_restore_script(snapshot: Path, root: Path, restore_path: Path) -> None:
    lines = []
    lines.append('param(')
    ...
    lines.append('Write-Host "=== V18.10B-R3 STABLE SNAPSHOT RESTORE ==="')
...
def main() -> int:
    ...
    stamp = now_stamp()
    snapshot_name = f"V18_10B_R3_stable_factor_research_chain_{stamp}"
    ...
    manifest_path = snapshot / "V18_10B_R3_STABLE_MANIFEST.csv"
    readme_path = snapshot / "V18_10B_R3_STABLE_SNAPSHOT_README.txt"
    restore_path = snapshot / "restore_v18_10B_R3_stable_snapshot.ps1"

    report_path = out_dir / "V18_10B_R3_CURRENT_STABLE_SNAPSHOT_REPORT.md"
    read_first_path = out_dir / "V18_10B_R3_READ_FIRST.txt"
    ...
    check_path = snapshot / "V18_10B_R3_STABLE_VALIDATION_CHECKS.csv"
    ...
    build_restore_script(snapshot=snapshot, root=root, restore_path=restore_path)

    # Capture the current key read-firsts for quick inspection.
    key_readfirsts = [
    ...
    for p in key_readfirsts:
        combined_rf.append("")
        combined_rf.append("=" * 80)
        combined_rf.append(str(p))
        combined_rf.append("=" * 80)
        combined_rf.append(read_text_safe(p))

    combined_rf_path = snapshot / "V18_10B_R3_COMBINED_READ_FIRST_CAPTURE.txt"
    ...
    readme_lines = []
    readme_lines.append("V18.10B-R3 STABLE SNAPSHOT README")
    readme_lines.append("")
    ...
    readme_lines.append("Stable restore point after V18.10B factor research daily chain became clean and non-black-box.")
    readme_lines.append("")
    ...
    report_lines = []
    report_lines.append("# V18.10B-R3 Stable Snapshot")
    report_lines.append("")
    ...
    report_lines.append("- MODE: `STABLE_RESTORE_POINT_FOR_FACTOR_RESEARCH_CHAIN`")
    ...
    read_first = f"""V18.10B-R3 STABLE SNAPSHOT READ FIRST

STATUS:
{status}

MODE:
STABLE_RESTORE_POINT_FOR_FACTOR_RESEARCH_CHAIN
...
NEXT_STEP:
If STATUS is OK_STABLE_SNAPSHOT_READY, proceed to V18.10C only after forward-return labels begin to mature.
"""
    read_first_path.write_text(read_first, encoding="utf-8")

    print("")
    print("=== V18.10B-R3 STABLE SNAPSHOT READY ===")
    print(f"STATUS: {status}")
    print("MODE: STABLE_RESTORE_POINT_FOR_FACTOR_RESEARCH_CHAIN")
