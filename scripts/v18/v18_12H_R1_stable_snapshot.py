...
MODE = "SNAPSHOT_ONLY"
STATUS_OK = "OK_V18_12H_R1_STABLE_SNAPSHOT_READY"
STATUS_WARN = "WARN_V18_12H_R1_STABLE_SNAPSHOT_WITH_FAILURES"
SHADOW_ONLY = "TRUE"
LINK_ONLY = "TRUE"
OFFICIAL_DECISION_IMPACT = "NONE"
AUTO_SELL = "DISABLED"
AUTO_TRADE = "DISABLED"
EXPECTED_LINK_STATUS = "OK_LINKED_TO_V18_12F_SELL_TIMING_READ_CENTER"
EXPECTED_H_STATUS = "OK_V18_12H_SELL_TIMING_READ_CENTER_LINK_READY"

SNAPSHOT_FILES = [
    ("script", "scripts/v18/run_v18_12H_sell_timing_read_center_link.ps1"),
    ("script", "scripts/v18/v18_12H_sell_timing_read_center_link.py"),
    ("output", "outputs/v18/read_center/V18_12H_READ_FIRST.txt"),
    ("output", "outputs/v18/read_center/V18_12H_CURRENT_SELL_TIMING_READ_LINK.md"),
    ("output", "outputs/v18/read_center/V18_12H_CURRENT_SELL_TIMING_READ_LINK_SUMMARY.csv"),
    ("output", "outputs/v18/read_center/V18_12H_CURRENT_SELL_TIMING_READ_LINK_INPUT_AUDIT.csv"),
]

PS_PARSE_FILES = [
    "scripts/v18/run_v18_12H_sell_timing_read_center_link.ps1",
    "scripts/v18/run_v18_12H_R1_stable_snapshot.ps1",
]

PY_COMPILE_FILES = [
    "scripts/v18/v18_12H_sell_timing_read_center_link.py",
    "scripts/v18/v18_12H_R1_stable_snapshot.py",
]
...
PROTECTED_FILES = [
    ...
    "scripts/v18/run_v18_12F_R2_stable_snapshot.ps1",
    "scripts/v18/v18_12F_R2_stable_snapshot.py",
]
...
SNAPSHOT_CHECKS = [
    ...
    ("V18_12F_R1_STABLE_SNAPSHOT_UNCHANGED", "V18_12F_R1_stable*", "V18_12F_R1_STABLE_MANIFEST.csv"),
    ("V18_12F_R2_STABLE_SNAPSHOT_UNCHANGED", "V18_12F_R2_stable*", "V18_12F_R2_STABLE_MANIFEST.csv"),
]

V18_12H_OUTPUTS = [
    "outputs/v18/read_center/V18_12H_READ_FIRST.txt",
    "outputs/v18/read_center/V18_12H_CURRENT_SELL_TIMING_READ_LINK.md",
    "outputs/v18/read_center/V18_12H_CURRENT_SELL_TIMING_READ_LINK_SUMMARY.csv",
    "outputs/v18/read_center/V18_12H_CURRENT_SELL_TIMING_READ_LINK_INPUT_AUDIT.csv",
]
...
def run_v18_12h_link(root: Path) -> Tuple[bool, str]:
    wrapper = root / "scripts/v18/run_v18_12H_sell_timing_read_center_link.ps1"
    try:
        proc = subprocess.run(
            [
                "powershell",
...
def no_order_tokens_in_outputs(root: Path) -> Tuple[bool, str]:
    hits: List[str] = []
    for rel_path in V18_12H_OUTPUTS:
        path = root / rel_path
...
Write-Host ""
Write-Host "=== V18.12H-R1 STABLE SNAPSHOT RESTORE ==="
Write-Host "SNAPSHOT: $Snapshot"
Write-Host "ROOT: $Root"
Write-Host "APPLY: $Apply"
Write-Host "NOTE: Restore covers only V18.12H sell timing read center link files and outputs."
...
def build_readme(snapshot: Path, manifest_path: Path, validation_path: Path, restore_path: Path) -> str:
    return f"""V18.12H-R1 STABLE SNAPSHOT README
...
Purpose:
Stable restore point for V18.12H Sell Timing Read Center Link after linking the latest V18.12F sell timing read center into outputs/v18/read_center.

Scope:
Copies the V18.12H wrapper, V18.12H read-link script, READ_FIRST, read-link Markdown, summary CSV, and input audit CSV.
...
SELL_TIMING_LINK_STATUS={EXPECTED_LINK_STATUS}

Restore:
The restore script is generated but not executed by snapshot creation. It restores only V18.12H files and generated V18.12H outputs.
...
def main() -> int:
    ...
    stamp = now_stamp()
    snapshot = root / "archive/stable" / f"V18_12H_R1_stable_sell_timing_read_center_link_{stamp}"
    ...
    manifest_path = snapshot / "V18_12H_R1_STABLE_MANIFEST.csv"
    validation_path = snapshot / "V18_12H_R1_STABLE_VALIDATION_CHECKS.csv"
    readme_path = snapshot / "V18_12H_R1_STABLE_SNAPSHOT_README.txt"
    restore_path = snapshot / "restore_v18_12H_R1_stable_snapshot.ps1"
    report_path = out_dir / "V18_12H_R1_CURRENT_STABLE_SNAPSHOT_REPORT.md"
    read_first_path = out_dir / "V18_12H_R1_READ_FIRST.txt"
    h_read_first = root / "outputs/v18/read_center/V18_12H_READ_FIRST.txt"

    validations: List[Dict[str, str]] = []
    wrapper_ok, wrapper_output = run_v18_12h_link(root)
    validations.append(validation_row("RUN_V18_12H_READ_CENTER_LINK", "PASS" if wrapper_ok else "FAIL", root / "scripts/v18/run_v18_12H_sell_timing_read_center_link.ps1", "exit_0", "exit_0" if wrapper_ok else "nonzero", wrapper_output))
    ...
    for key, expected in read_first_expectations:
        actual = read_first_value(h_read_first, key)
        h_values[key] = actual
        validations.append(validation_row(f"V18_12H_{key}", "PASS" if actual == expected else "FAIL", h_read_first, expected, actual))

    no_token_ok, token_note = no_order_tokens_in_outputs(root)
    validations.append(validation_row("NO_ORDER_TOKEN_IN_V18_12H_OUTPUTS", "PASS" if no_token_ok else "FAIL", "outputs/v18/read_center/V18_12H*", "no immediate live sell tokens", "clean" if no_token_ok else "found", token_note))
    ...
    report = f"""# V18.12H-R1 Stable Snapshot
...
- SELL_TIMING_LINK_STATUS: `{h_values.get('SELL_TIMING_LINK_STATUS', '')}`
- OFFICIAL_DECISION_IMPACT: `{OFFICIAL_DECISION_IMPACT}`
- AUTO_SELL: `{AUTO_SELL}`
- AUTO_TRADE: `{AUTO_TRADE}`
- SHADOW_ONLY: `{SHADOW_ONLY}`
...
The snapshot contains only V18.12H sell timing read center link files, generated V18.12H outputs, README, manifest, validation checks, and restore tooling.
...
The V18.12H wrapper was run before copying. PowerShell parse checks, Python compile checks, critical file copy checks, protected-file hash checks, V18.12A/B/C/D/E/F-R1 and F-R2 stable snapshot manifest hash checks, and generated output token checks were recorded.
"""
    report_path.write_text(report, encoding="utf-8")

    read_first_out = f"""V18.12H-R1 STABLE SNAPSHOT READ FIRST
...
{h_values.get('SELL_TIMING_LINK_STATUS', '')}
...
"""
    ...
    print(f"SELL_TIMING_LINK_STATUS: {h_values.get('SELL_TIMING_LINK_STATUS', '')}")
