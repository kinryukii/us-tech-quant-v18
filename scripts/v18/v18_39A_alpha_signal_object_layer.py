...
SUMMARY_COLUMNS = [
    ...
    "top_full_ticker_overlap_count",
    "top_full_ticker_mismatch_count",
    "top_full_order_matches",
    "top_full_canonical_sync_required",
    "status",
...
def canonical_top20_from_full(full: pd.DataFrame) -> list[str]:
    if full.empty:
        return []
    ...
    if not ticker_col:
        return []
    work = full.copy()
    rank_col = find_col(work, "rank", "source_rank")
    score_col = find_col(work, "composite_candidate_score")
    work["_ticker_norm"] = work[ticker_col].astype(str).str.upper().str.strip()
    work = work[work["_ticker_norm"] != ""].copy()
    if rank_col:
        work["_rank_numeric"] = work[rank_col].apply(to_float)
        if work["_rank_numeric"].notna().all():
            work = work.sort_values(["_rank_numeric", "_ticker_norm"], ascending=[True, True])
        elif score_col:
            work["_score_numeric"] = work[score_col].apply(to_float)
            work = work.sort_values(["_score_numeric", "_ticker_norm"], ascending=[False, True])
    elif score_col:
        work["_score_numeric"] = work[score_col].apply(to_float)
        work = work.sort_values(["_score_numeric", "_ticker_norm"], ascending=[False, True])
    return work["_ticker_norm"].head(20).tolist()


def top_tickers(top: pd.DataFrame) -> list[str]:
    if top.empty:
        return []
    ticker_col = find_col(top, "ticker", "yf_ticker", "symbol")
    if not ticker_col:
        return []
    return [str(v).upper().strip() for v in top[ticker_col].head(20).tolist() if str(v).strip()]


def top_full_diagnostics(full: pd.DataFrame, top: pd.DataFrame) -> dict[str, Any]:
    canonical = canonical_top20_from_full(full)
    current_top = top_tickers(top)
    overlap = len(set(canonical) & set(current_top))
    order_matches = canonical == current_top
    only_current = [ticker for ticker in current_top if ticker not in set(canonical)]
    only_full = [ticker for ticker in canonical if ticker not in set(current_top)]
    mismatch_count = max(len(only_current), len(only_full))
    if mismatch_count == 0 and not order_matches:
        mismatch_count = sum(1 for left, right in zip(current_top, canonical) if left != right)
    return {
        "top_full_ticker_overlap_count": overlap,
        "top_full_ticker_mismatch_count": mismatch_count,
        "top_full_order_matches": str(order_matches).upper(),
        "top_full_canonical_sync_required": str(mismatch_count > 0 or not order_matches).upper(),
    }
...
def build_objects(root: Path) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any], list[str]]:
    ...
    top_full = top_full_diagnostics(full, top if top_status == "OK" else pd.DataFrame())
    top_full_mismatch = top_full.get("top_full_canonical_sync_required") == "TRUE"
    ...
    for _, row in full.iterrows():
        ...
        output_data_quality_tags = list(data_quality_tags)
        if top_full_mismatch:
            output_data_quality_tags.append("TOP_FULL_CANDIDATE_SOURCE_MISMATCH")
        ...
        row_out = {
            ...
            "data_quality_tags": join_tags(output_data_quality_tags),
...
def build_summary(objects: pd.DataFrame, context: dict[str, Any], warnings: list[str]) -> dict[str, Any]:
    ...
    elif context.get("top_full_canonical_sync_required") == "TRUE":
        status = "WARN_V18_39A_ALPHA_SIGNAL_OBJECT_LAYER_TOP_FULL_SYNC_REQUIRED"
        next_step = "Run V18.40A apply mode before using the buy-candidate report: scripts/v18/run_v18_40A_candidate_top_full_canonical_sync.ps1 -ApplyCandidateTopFullCanonicalSync"
    ...
    return {
        ...
        "top_full_ticker_overlap_count": context.get("top_full_ticker_overlap_count", ""),
        "top_full_ticker_mismatch_count": context.get("top_full_ticker_mismatch_count", ""),
        "top_full_order_matches": context.get("top_full_order_matches", ""),
        "top_full_canonical_sync_required": context.get("top_full_canonical_sync_required", ""),
        "status": status,
...
- TOP/FULL ticker overlap count: {summary.get('top_full_ticker_overlap_count', '')}
- TOP/FULL ticker mismatch count: {summary.get('top_full_ticker_mismatch_count', '')}
- TOP/FULL order matches: {summary.get('top_full_order_matches', '')}
- TOP/FULL canonical sync required: {summary.get('top_full_canonical_sync_required', '')}
...
def build_read_first(summary: dict[str, Any], context: dict[str, Any], run_id: str, generated_at: str) -> str:
    fields = {
        ...
        "TOP_FULL_TICKER_OVERLAP_COUNT": summary.get("top_full_ticker_overlap_count", ""),
        "TOP_FULL_TICKER_MISMATCH_COUNT": summary.get("top_full_ticker_mismatch_count", ""),
        "TOP_FULL_ORDER_MATCHES": summary.get("top_full_order_matches", ""),
        "TOP_FULL_CANONICAL_SYNC_REQUIRED": summary.get("top_full_canonical_sync_required", ""),
        "COMMAND_STATUS_CURRENT_BLOCKING_COUNT": context.get("command_current_blocking_count", ""),
        "DAILY_RUN_USABLE": context.get("daily_run_usable", ""),
