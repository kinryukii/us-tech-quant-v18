...
STATUS_OK = "OK_V18_22A_STABLE_SNAPSHOT_READY"
STATUS_WARN = "WARN_V18_22A_STABLE_SNAPSHOT_VALIDATION_FAILED"
MODE = "SNAPSHOT_ONLY"
SNAPSHOT_ONLY = "TRUE"
PREFIX = "V18_22A_stable_research_command_center"
...
REQUIRED_FILES = [
    "scripts/v18/v18_22A_stable_snapshot.py",
    "scripts/v18/run_v18_22A_stable_snapshot.ps1",
...
READ_FIRST_FIELDS = [
    "STATUS", "MODE", "SNAPSHOT_ONLY", "POLICY_APPLIED", "RESEARCH_COMMAND_CENTER_READY",
...
def readme(metrics: Dict[str, object], snapshot: Path) -> str:
    return f"""# V18.22A Stable Snapshot

This snapshot preserves V18.22A Research Command Center.

Required interpretation:
- This is advisory read-center-only.
- It summarizes {metrics.get('STABLE_LAYER_COUNT', '9')} stable research layers.
- It is not production integration.
- Daily command center integration remains disabled.
- Price cache integration remains disallowed.
- Staged backfill actual fetch/import requires explicit approval.
- No external data was fetched.
- No price cache was modified.
...
def report(metrics: Dict[str, object], snapshot: Path) -> str:
    return f"""# V18.22A Stable Snapshot Report

## Executive Summary
Status: {metrics.get('STATUS')}. This snapshot preserves the Research Command Center and read-center-only state.

## Preserved Command Center State
Research command center ready: {metrics.get('RESEARCH_COMMAND_CENTER_READY')}. Stable layers: {metrics.get('STABLE_LAYER_OK_COUNT')} of {metrics.get('STABLE_LAYER_COUNT')}. Recommended next action: {metrics.get('RECOMMENDED_NEXT_ACTION')}.

## Gates Preserved
Factor claims: {metrics.get('FACTOR_EFFECT_CLAIM_ALLOWED')}. Weight changes: {metrics.get('WEIGHT_CHANGE_ALLOWED')}. Production promotion: {metrics.get('PRODUCTION_PROMOTION_ALLOWED')}. Staged fetch/import: {metrics.get('STAGED_BACKFILL_ACTUAL_FETCH_IMPORT_ALLOWED')}. Price cache integration: {metrics.get('PRICE_CACHE_INTEGRATION_ALLOWED')}. Daily command center integration: {metrics.get('DAILY_COMMAND_CENTER_INTEGRATION_ALLOWED')}.
...
def restore_script(files: Sequence[str]) -> str:
    lines = [
        ...
        'Write-Host "=== RESTORE V18.22A STABLE SNAPSHOT START ==="',
        'Write-Host "MODE: SNAPSHOT_RESTORE"',
        'Write-Host "NOTE: This restores advisory read-center command center artifacts only."',
    ]
    for rel in files:
        win = rel.replace("/", "\\")
        lines.extend([
...
def build_manifest(root: Path, snapshot: Path) -> List[Dict[str, object]]:
    ...
    manifest.append(snapshot_artifact_row(snapshot, "RESTORE_V18_22A.ps1", "RESTORE"))
    manifest.append(snapshot_artifact_row(snapshot, "README_V18_22A_STABLE_SNAPSHOT.md", "README"))
...
def main() -> int:
    ...
    read_first_path = root / "outputs/v18/ops/V18_22A_STABLE_READ_FIRST.txt"
    report_path = root / "outputs/v18/ops/V18_22A_CURRENT_STABLE_SNAPSHOT_REPORT.md"
    manifest_path = snapshot / "MANIFEST.csv"
    validation_path = snapshot / "VALIDATION.csv"
    readme_path = snapshot / "README_V18_22A_STABLE_SNAPSHOT.md"
    restore_path = snapshot / "RESTORE_V18_22A.ps1"

    metrics = readfirst(root / "outputs/v18/ops/V18_22A_READ_FIRST.txt")
    ...
    initial_manifest.append(snapshot_artifact_row(snapshot, "RESTORE_V18_22A.ps1", "RESTORE"))
    initial_manifest.append(snapshot_artifact_row(snapshot, "README_V18_22A_STABLE_SNAPSHOT.md", "README"))
    ...
    for rel in OPTIONAL_FILES:
        path = root / rel
        validations.append(validation("supporting_stable_read_first_exists_if_available", path.exists(), str(path), "EXISTS_IF_AVAILABLE", "EXISTS" if path.exists() else "MISSING"))
    for key, expected in {
        "RESEARCH_COMMAND_CENTER_READY": "TRUE",
        "EXTERNAL_DATA_FETCHED": "FALSE",
        "PRICE_CACHE_MODIFIED": "FALSE",
