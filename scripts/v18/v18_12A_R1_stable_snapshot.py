...
MODE = "SNAPSHOT_ONLY"
STATUS_OK = "OK_V18_12A_R1_STABLE_SNAPSHOT_READY"
...
SNAPSHOT_FILES = [
    ("script", "scripts/v18/v18_12A_sell_timing_shadow_engine.py"),
    ("script", "scripts/v18/run_v18_12A_sell_timing_shadow_engine.ps1"),
    ("output", "outputs/v18/sell_timing/V18_12A_READ_FIRST.txt"),
    ("output", "outputs/v18/sell_timing/V18_12A_CURRENT_SELL_TIMING_SHADOW_REPORT.md"),
    ("output", "outputs/v18/sell_timing/V18_12A_CURRENT_SELL_TIMING_SHADOW.csv"),
    ("output", "outputs/v18/sell_timing/V18_12A_CURRENT_SELL_TIMING_INPUT_AUDIT.csv"),
]
...
PY_COMPILE_FILES = [
    "scripts/v18/v18_12A_sell_timing_shadow_engine.py",
...
Write-Host ""
Write-Host "=== V18.12A-R1 STABLE SNAPSHOT RESTORE ==="
Write-Host "SNAPSHOT: $Snapshot"
Write-Host "ROOT: $Root"
Write-Host "APPLY: $Apply"
Write-Host "NOTE: Restore covers only V18.12A sell timing shadow files."
...
def build_readme(snapshot: Path, manifest_path: Path, validation_path: Path, restore_path: Path) -> str:
    return f"""V18.12A-R1 STABLE SNAPSHOT README
...
Purpose:
Stable restore point for the V18.12A Sell Timing Shadow Engine after documentation improvement and validation.

Scope:
Copies the V18.12A sell timing engine, wrapper, READ_FIRST, report, shadow CSV, and input audit CSV.
...
Restore:
The restore script is generated but not executed by snapshot creation. It restores only V18.12A sell timing shadow files.
...
def main() -> int:
    ...
    stamp = now_stamp()
    snapshot = root / "archive/stable" / f"V18_12A_R1_stable_sell_timing_shadow_engine_{stamp}"
    ...
    manifest_path = snapshot / "V18_12A_R1_STABLE_MANIFEST.csv"
    validation_path = snapshot / "V18_12A_R1_STABLE_VALIDATION_CHECKS.csv"
    readme_path = snapshot / "V18_12A_R1_STABLE_SNAPSHOT_README.txt"
    restore_path = snapshot / "restore_v18_12A_R1_stable_snapshot.ps1"
    report_path = out_dir / "V18_12A_R1_CURRENT_STABLE_SNAPSHOT_REPORT.md"
    read_first_path = out_dir / "V18_12A_R1_READ_FIRST.txt"
    ...
    read_first = root / "outputs/v18/sell_timing/V18_12A_READ_FIRST.txt"
    for key, expected in [
        ("OFFICIAL_DECISION_IMPACT", OFFICIAL_DECISION_IMPACT),
        ("AUTO_SELL", AUTO_SELL),
        ("AUTO_TRADE", AUTO_TRADE),
        ...
        validations.append(validation_row(f"V18_12A_{key}", "PASS" if actual == expected else "FAIL", read_first, expected, actual))
    ...
    status = STATUS_OK if fail_count == 0 else "WARN_V18_12A_R1_STABLE_SNAPSHOT_WITH_FAILURES"

    report = f"""# V18.12A-R1 Stable Snapshot
...
The snapshot contains only V18.12A sell timing shadow engine files, generated V18.12A sell timing outputs, README, manifest, validation checks, and restore tooling.
...
PowerShell parse checks and Python compile checks were run for V18.12A and V18.12A-R1 scripts. Critical copied files were confirmed. The official daily script hash was checked before and after snapshot creation.
"""
    report_path.write_text(report, encoding="utf-8")

    read_first_out = f"""V18.12A-R1 STABLE SNAPSHOT READ FIRST
