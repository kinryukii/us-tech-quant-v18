...
MODE = "SNAPSHOT_ONLY"
STATUS_OK = "OK_V18_12C_R1_STABLE_SNAPSHOT_READY"
...
SNAPSHOT_FILES = [
    ("script", "scripts/v18/v18_12C_position_lifecycle_review.py"),
    ("script", "scripts/v18/run_v18_12C_position_lifecycle_review.ps1"),
    ("output", "outputs/v18/sell_timing/V18_12C_READ_FIRST.txt"),
    ("output", "outputs/v18/sell_timing/V18_12C_CURRENT_POSITION_LIFECYCLE_REVIEW_REPORT.md"),
    ("output", "outputs/v18/sell_timing/V18_12C_CURRENT_POSITION_LIFECYCLE_REVIEW.csv"),
    ("output", "outputs/v18/sell_timing/V18_12C_CURRENT_POSITION_LIFECYCLE_INPUT_AUDIT.csv"),
]
...
PY_COMPILE_FILES = [
    "scripts/v18/v18_12C_position_lifecycle_review.py",
    "scripts/v18/v18_12C_R1_stable_snapshot.py",
]

V18_12C_OUTPUTS = [
    "outputs/v18/sell_timing/V18_12C_READ_FIRST.txt",
    "outputs/v18/sell_timing/V18_12C_CURRENT_POSITION_LIFECYCLE_REVIEW_REPORT.md",
    "outputs/v18/sell_timing/V18_12C_CURRENT_POSITION_LIFECYCLE_REVIEW.csv",
]

DISALLOWED_LIVE_SELL_TOKENS = [
    "SELL_NOW",
    "LIVE_SELL",
    "AUTO_SELL_NOW",
    "MARKET_SELL",
...
def run_v18_12c_wrapper(root: Path) -> Tuple[bool, str]:
    wrapper = root / "scripts/v18/run_v18_12C_position_lifecycle_review.ps1"
    try:
        proc = subprocess.run(
...
def no_live_sell_tokens_in_outputs(root: Path) -> Tuple[bool, str]:
    hits: List[str] = []
    for rel_path in V18_12C_OUTPUTS:
        path = root / rel_path
        text = read_text(path).upper()
        for token in DISALLOWED_LIVE_SELL_TOKENS:
            if token in text:
...
Write-Host ""
Write-Host "=== V18.12C-R1 STABLE SNAPSHOT RESTORE ==="
Write-Host "SNAPSHOT: $Snapshot"
Write-Host "ROOT: $Root"
Write-Host "APPLY: $Apply"
Write-Host "NOTE: Restore covers only V18.12C position lifecycle review files."
...
def build_readme(snapshot: Path, manifest_path: Path, validation_path: Path, restore_path: Path) -> str:
    return f"""V18.12C-R1 STABLE SNAPSHOT README
...
Purpose:
Stable restore point for the additive V18.12C Position Lifecycle Review layer.

Scope:
Copies the V18.12C lifecycle review engine, wrapper, READ_FIRST, report, lifecycle CSV, and lifecycle input audit CSV.
...
Restore:
The restore script is generated but not executed by snapshot creation. It restores only V18.12C position lifecycle review files.
...
def main() -> int:
    ...
    stamp = now_stamp()
    snapshot = root / "archive/stable" / f"V18_12C_R1_stable_position_lifecycle_review_{stamp}"
    ...
    manifest_path = snapshot / "V18_12C_R1_STABLE_MANIFEST.csv"
    validation_path = snapshot / "V18_12C_R1_STABLE_VALIDATION_CHECKS.csv"
    readme_path = snapshot / "V18_12C_R1_STABLE_SNAPSHOT_README.txt"
    restore_path = snapshot / "restore_v18_12C_R1_stable_snapshot.ps1"
    report_path = out_dir / "V18_12C_R1_CURRENT_STABLE_SNAPSHOT_REPORT.md"
    read_first_path = out_dir / "V18_12C_R1_READ_FIRST.txt"

    validations: List[Dict[str, str]] = []
    wrapper_ok, wrapper_output = run_v18_12c_wrapper(root)
    validations.append(validation_row("RUN_V18_12C_WRAPPER", "PASS" if wrapper_ok else "FAIL", root / "scripts/v18/run_v18_12C_position_lifecycle_review.ps1", "exit_0", "exit_0" if wrapper_ok else "nonzero", wrapper_output))
    ...
    read_first = root / "outputs/v18/sell_timing/V18_12C_READ_FIRST.txt"
    for key, expected in [
        ...
        validations.append(validation_row(f"V18_12C_{key}", "PASS" if actual == expected else "FAIL", read_first, expected, actual))

    no_token_ok, token_note = no_live_sell_tokens_in_outputs(root)
    validations.append(validation_row("NO_LIVE_SELL_TOKEN_IN_V18_12C_OUTPUTS", "PASS" if no_token_ok else "FAIL", "outputs/v18/sell_timing/V18_12C*", "no live sell tokens", "clean" if no_token_ok else "found", token_note))
    ...
    status = STATUS_OK if fail_count == 0 else "WARN_V18_12C_R1_STABLE_SNAPSHOT_WITH_FAILURES"

    report = f"""# V18.12C-R1 Stable Snapshot
...
The snapshot contains only V18.12C position lifecycle review files, generated V18.12C outputs, README, manifest, validation checks, and restore tooling.
...
The V18.12C wrapper was run before copying. PowerShell parse checks, Python compile checks, critical file copy checks, official daily hash check, V18.12A/V18.12B stable snapshot manifest hash checks, and immediate live-sell token checks were recorded.
"""
    report_path.write_text(report, encoding="utf-8")

    read_first_out = f"""V18.12C-R1 STABLE SNAPSHOT READ FIRST
